...
| req-grid-reconcile-verb-9 | Contradiction fenced at apply | Implemented | A tombstone whose contained closure holds a node observed by any committed batch since the candidate record was derived is `contradicted` at apply, the closure rows locked first; nothing is written and the record names the descendants. | Issue# 656 - tap. `TestContradiction::test_a_descendant_observed_after_derivation_fences_the_tombstone`; `applied.contradicted` sums them; log `[2611]`. |
