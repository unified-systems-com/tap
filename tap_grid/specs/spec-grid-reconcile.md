...
| req-grid-reconcile-candidates | [Candidate Derivation And Prerequisites](#candidate-derivation-and-prerequisites) | Implemented | Fan-out from the parent; prerequisite read per parent; withdrawal is not absence — derived and recorded on the run's Batch, authority off |
| req-grid-reconcile-falsifier | [Per-Type Falsifiers And Their Verdicts](#per-type-falsifiers-and-their-verdicts) | Implemented | Manifest-registered probe; five verdicts; probe compares identity and owner, not HTTP status; no falsifier means not reconcilable. Slice 3 landed the contract, registry, classification and the recorded verdicts (authority off); the writes are the verb's (slice 4) |
| req-grid-reconcile-verb | [Service-Owned Reconciliation](#service-owned-reconciliation) | Implemented | One `reconcile` verb, run-config authority default off, budget, fence against stale verdicts. Slice 4 (Issue# 652 - tap): `grid.reconcile` capability, per-collector authority and budget, verdicts applied behind the fence, collectors never retire |
...
#### Status Details
Implemented 2026-09-18 (phase 4 slice 2, Issue# 596 - tap; authority off — derive and record, retire nothing). `tap_grid.candidates.derive_candidates` is the one derivation; `record_candidates` validates the result against `tap_grid/schemas/candidates.schema.json` (every field described) and stores it on the run's lifecycle `Batch` under `metadata["candidates"]`, beside the completeness statement it is derived from; `candidates_of` is the read, surfaced by `batch_summary()` and the batch viewer panel. The run task (`tap_cares/tasks.py`) records it right after the statement on both terminal paths, best-effort and logged, comparing scope against the collector's previous successful run. The three inputs are all already on the record — the declared containment (`CONTAINMENT_EDGES`, read through `tap.edge_declarations`), the run's own committed write batches (`observed(this run)` is every entity with a create/update `BatchEvent` on one of them; the service write pipeline records that event for every node it writes, unchanged re-observations included), and the completeness statement (per-parent prerequisite = the parent is live, was itself observed this run, and its surface is `reconcilable` by the statement's own derivation). The spec's "collector's descent table" has no code: the completeness surface carries the producer's mapping from its relation to the grid edge type in a new optional, described `edge_type` field, and a surface with no mapping, or one mapped to a type the parent's model does not declare as containment, yields nothing. Resolves two contradictions present in the design's first draft, both recorded in the design document. Open beside it: the cascade's containment reader is private to the delete pipeline, so the derivation reads the same declaration through a second public reader — one shared reader is Issue# 598 - tap; withdrawal compares scope per `(subject, relation)`, so a narrowed *filter* on a still-authorized surface is not (yet) a withdrawal — the ruling is Issue# 599 - tap.
...
**Contradiction — ruled 2026-09-19 (Issue# 656 - tap).** A candidate exists because this run did not observe it; a contained cascade from it would retire everything under it. If the same run observed a node *under* the candidate — a create or update in one of its committed batches — both cannot be true of the source at once: the collector's enumeration missed the parent, the source deleted it mid-collection, or the child belongs to an owner the directed graph does not model (Issue# 649 - tap). All three want a human, not a cascade. So derivation checks each candidate's contained closure — read through the service layer's own cascade discovery, `tap_grid.services.contained_closure`, so "under it" is the cascade's definition — against the run's observed set and records the result on the candidate: `contradiction: {kind, observed_descendants, count}`, or null. A closure that exceeds the cascade cap is a contradiction of its own kind, `closure_unknown`: unknown is refused, never passed. A contradicted candidate is refused by every later stage — the dispatch records it `contradicted` without asking a falsifier or spending budget, whatever the authority, and the verb never applies it — and resolved by none of them. The verb adds a second fence at apply (`req-grid-reconcile-verb-9`): the closure and the observations can both move between derivation and apply.

**The record.** One entry per surface examined: this run's surfaces in statement order, then every surface of the previous statement this run's scope no longer holds. Each names the parent it fanned out from, an `outcome` (`derived` / `withdrawn` / `skipped`), a code-first `reason` when not derived (`parent_retired`, `parent_not_observed`, `surface_not_reconcilable`, `relation_not_containment`, `relation_unmapped`, `subject_unresolved`, `parent_unknown`, `fan_out_exceeds_cap`, `scope_withdrawn`), the fan-out and observed counts, and the candidates — each with its entity type and the closed-vocabulary reason it *would* carry (`dropped_from_observation` or `scope_withdrawn`). `previous` is `null` when no earlier run was offered: withdrawal not observable, which is a different fact from nothing withdrawn. `authority` is the constant `off`.
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-grid-reconcile-candidates-1 | Fan-out derivation | Implemented | Candidates are derived from declared containment relations in the grid minus the run's observed set; no per-node stamp is read. | `tap_grid/tests/test_reconcile_candidates.py::TestFanOutDerivation` (c1, c2 observed, c3 the candidate; the derivation's captured SQL touches no typed row and no `flip_map`; an unchanged re-observation counts; a failed write batch is not an observation); end to end in `tap_cares/tests/test_candidates_flow.py::TestCandidatesReachTheRun` (the second run names the child the first saw and it did not). |
| req-grid-reconcile-candidates-2 | Prerequisite per parent | Implemented | A failed sibling-listing for parent Q does not prevent falsification of parent P's children when P was observed and P's child surface completed. | Directly from the review's acceptance cases. `TestPrerequisitePerParent`: Q's surface not reconcilable → Q skipped, P derived; a parent this run did not observe yields nothing however complete its surface reads; an absent sibling surface is not consulted. |
| req-grid-reconcile-candidates-3 | Ordering preserved | Implemented | Children are never falsified after their parent has been retired; a retired parent's children are handled by cascade. | `TestOrderingPreserved`: a tombstoned parent is skipped `parent_retired` before its children are read, for derivation and for withdrawal alike. |
| req-grid-reconcile-candidates-4 | Withdrawal distinguished | Implemented | A previously observed child excluded by a narrowed scope is retired with reason `scope_withdrawn`; a never-observed out-of-scope child is not a candidate. | `TestWithdrawalDistinguished`: q1, q2 (observed by the previous run) are `scope_withdrawn` candidates and an operator-added never-observed child is not; a surface refused this run is a withdrawal, not an absence; no previous run → `previous: null`; a previous run without a statement compares nothing. Filter-level narrowing: Issue# 599 - tap. |
| req-grid-reconcile-candidates-6 | Contradiction refuses | Implemented | A candidate whose contained closure holds a node this run observed live is recorded `contradiction: {observed_descendants, count}` on the candidate record; the dispatch records it `contradicted` — no falsifier asked, no budget spent, whatever the authority — and nothing retires it; a closure over the cascade cap is `closure_unknown` and refused the same way. | Issue# 656 - tap. `test_reconcile_candidates.py::TestContradiction` (observed grandchild → contradiction with the ids and count; none → null; cap → `closure_unknown`; the closure gateway); `test_reconcile_verb.py::TestContradiction` (never probed, no budget used; visible with authority off). Corpus family `reconcile` (`tap_grid/cascade_corpus/scenarios/reconcile.cascade.json`, Issue# 662 - tap): an observed grandchild → `contradicted` with the ids; a closure over the cap → `closure_unknown`; a dropped candidate with a clean closure → `applied` with its cascade; a present candidate → `not_applicable`; the model oracle (`model_oracle.reconcile`) and the runner both check each. |
| req-grid-reconcile-candidates-5 | References excluded | Implemented | A node reachable from the parent only by a reference relation never appears in a candidate set. | `TestReferencesExcluded`: `r` (ALT_LINK) is never in P's set; a surface mapped to a reference edge type, or to none, is skipped and yields nothing, for derivation and withdrawal alike. |
...
#### Status Details
Partial. Shape agreed 2026-09-14; mechanics agreed 2026-09-15. **Phase 4 slice 3 landed 2026-09-18 (Issue# 644 - tap), authority off:** the verdict vocabulary and the probe classification (`tap_grid/falsifiers.py::classify`), the `Falsifier` base class with the batch-over-singular default and the per-type registry, the manifest `[falsifiers]` table (`req-tap-plugin-manifest-v0-falsifiers`), the dispatch `falsify_candidates` that runs each type's falsifier ONCE over that type's candidates and records the verdicts on the run's `Batch` beside its candidate record (`tap_grid/schemas/verdicts.schema.json`, surfaced by `batch_summary` and the batch viewer), the coverage warning in `validate_plugin`, and the four-case proof harness (`tap_grid/falsifier_testing.py`). Every recorded entry names the write the verb WOULD make and its one home (-3) without making it. **Slice 4 landed the writes** (`req-grid-reconcile-verb`, Issue# 652 - tap): the tombstone on `DROPPED_FROM_OBSERVATION`, the name update on a rename, the ownership-edge retirement on a transfer, each behind the fence. Edge re-derivation (-5) remains Proposed.
...
**Known limitation — ownership is a directed graph (ruled 2026-09-19, "make it work").** Every consumer of ownership reads the one `CONTAINMENT_EDGES` declaration: the cascade walks it, candidate derivation fans out along it, and this requirement's owner comparison takes "the owner the grid saw it under" to be the parent on the listing, in source terms. Where the grid holds **no** owner for that parent — a keyless or synthesised parent, or a keyed row whose source identity was never stored — a probe-reported owner is **not** a transfer: there is no "this owner" whose edge could be ended, so the object is `PRESENT_AT_PROBE` and the owner is new information (**Option A**, Issue# 650 - tap; `owner_not_compared`). The entry carries the note, the dispatch logs a warning, and the record counts it (`ownership_not_compared`), so the corner case is visible wherever the record is read. The symmetric case — the grid holds an owner, the probe reports none — stays a refusal (`incomplete`). The cascade corpus's `ownership` family pins the same reading on the delete side. When a real relationship stops fitting a directed edge from one parent, the answer is deterministic per-type ownership logic with the declaration as the default — the Gryphon-with-fallback pattern — held as Issue# 649 - tap until a case exists.
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-grid-reconcile-falsifier-1 | No falsifier, no retirement | Implemented | A candidate of a type with no registered falsifier is recorded as not-re-observed and is never retired. | `test_reconcile_falsifier.py::TestDispatch::test_no_falsifier_means_not_reconcilable_and_nothing_probed`: the entry is `not_reconcilable`, no probe runs, the grid is unchanged. |
| req-grid-reconcile-falsifier-2 | Coverage is visible | Implemented | Plugin validation reports every model of a reconcilable kind that declares no falsifier. | Ratchet, not a gate: `validate_plugin` `falsifier-coverage` warns per containment-target model without a `[falsifiers]` row; `--strict` promotes. `tap_plugins/tests/test_manifest_falsifiers.py::TestValidateChecks`. |
| req-grid-reconcile-falsifier-3 | Verdicts have one home each | Implemented | `DROPPED_FROM_OBSERVATION` tombstones the **entity**. The scope-ending half of `RELOCATED` retires the **ownership edge** and leaves the entity live. `PRESENT_AT_PROBE`, `UNDETERMINED` and `REIDENTIFIED` write run records only. | Clarified 2026-09-15: "write to the entity" read as though a transfer could tombstone the row. Slice 3 records the home per entry (`would.write` / `would.home`, `test_would_names_one_home_per_verdict`); the write itself is slice 4. Write half landed with the verb (slice 4): `test_reconcile_verb.py::TestApplying` — the tombstone on the entity, the edge on a transfer, run record only for the rest. |
| req-grid-reconcile-falsifier-4 | Present-at-probe claims no defect | Implemented | A `PRESENT_AT_PROBE` record states the disjunction and names a cause of `created_after_listing_started` or `indeterminate`; it never asserts a collector defect, and frequency is not used to classify cause. | Review acceptance case: object appears between listing and probe. `TestPresentAtProbeClaimsNoDefect` (timing rules a cause in, never restored access out) and `TestDispatch::test_present_at_probe_is_recorded_with_the_disjunction_and_a_cause`. |
| req-grid-reconcile-falsifier-7 | Probe compares identity and owner | Implemented | A probe returning success for a candidate under a different owner ends that owner's relationship rather than recording presence; one returning a different source identity at the same address retires the old observation. | Otherwise a transferred object stays attached to its former owner. Classification landed (`TestClassification`: transfer → `RELOCATED(transferred)`, a different identity → `REIDENTIFIED`, never presence); the writes are slice 4. Write half landed with the verb: a transfer ends this parent's containment edge (`test_a_transfer_ends_this_parents_edge_only_and_cascades_nothing`). |
| req-grid-reconcile-falsifier-8 | Rename is a locator update | Implemented | A probe returning the same source identity under a new name updates the name and retires nothing; no alias machinery is required. | Classification landed (`RELOCATED(renamed)`, would-write `rename` on `entity_name`); the update is slice 4. Write half landed with the verb: `test_a_rename_updates_the_name_and_retires_nothing`. |
| req-grid-reconcile-falsifier-5 | Derived edges re-derive | Proposed | An edge derived from declared content is ended by re-derivation when its source content is gone, not by a probe. | Slice 4 or later; not part of slice 3. |
| req-grid-reconcile-falsifier-9 | Transfer retires nothing and cascades nothing | Implemented | A candidate whose probe returns the same source identity under a different owner retires the ownership edge only: the entity stays live, its contained children stay live, and no cascade runs — asserted on a parent with children rather than on a leaf. | Directly from the review's transfer finding; the failure it guards is a subtree tombstoned after being observed under its new owner. Slice 3 records `end_ownership_edge` on `ownership_edge` for a transfer and asserts the grid untouched (`test_a_transfer_would_end_the_ownership_edge_only`); the parent-with-children assertion belongs with the write, slice 4. Asserted on a parent with children in `test_a_transfer_ends_this_parents_edge_only_and_cascades_nothing`: the entity and its child stay live, no delete event, only the edge moved. |
| req-grid-reconcile-falsifier-6 | Four proof cases | Implemented | Each registered falsifier has tests for present, dropped, forbidden and reidentified against a fake source. | The harness is `tap_grid/falsifier_testing.py::run_four_cases` (one `batch_falsify` call, four asserted verdicts) with `FakeSource` / `FakeSourceFalsifier` as the reference pair; `TestFourProofCases` proves the harness and the reference. Each plugin runs the harness over its own falsifier and fake client — that per-plugin obligation is what `falsifier-coverage` makes visible. |
...
#### Status Details
Implemented — phase 4 slice 4 (Issue# 652 - tap), on the write side of `req-grid-service-delete-reason` and `req-grid-service-delete-cascade`. **The verb** is `tap_grid.services.reconcile(run_batch)`, gated by `grid.reconcile` (`tap_auth/tap_auth.capabilities.json`; a write-scope capability distinct from `grid.delete`, held by the `tap_cares.collector` program actor) and bodied by `tap_grid/reconcile.py::reconcile_run`. **Authority and budget are the run's configuration, carried into its lifecycle batch when the run is opened** (`tap_grid.reconcile.run_config` passed to `create_batch` by `tap_cares.services.run_collection` before any collector code executes — no writer of the configuration exists afterwards, and `Batch` is `INTERNAL_ONLY`, from `Collector.reconcile_authority` — default false — and `reconcile_budget`, null → the collector class's `RECONCILE_BUDGET_DEFAULT`). The verb reads them from the stamp and **nowhere else**: it takes no authority or budget argument, so holding `grid.reconcile` licenses running the verb on a run, never arming one — collector code running as the same actor cannot arm its own run, and the collectors-never-retire walk forbids `reconcile`, `write_batch`, the internal bypasses and the test harness's `arm_run_for_tests` in collector modules too. The batches whose observations count as this run's own come from the candidate record's `observed_batches`, also derived, never supplied. `Collector` is `INTERNAL_ONLY` (`req-tap-cares-collector-model-9`, pinned), so the switch is set through the internal patch path by an operator (an operator-facing command is Issue# 655 - tap); a boot-profile switch can layer on later. **The hook** is `tap_cares/tasks.py::_reconcile`, the run's final phase on the SUCCESSFUL path only (a failed run's evidence licenses nothing). With authority off the verb probes nothing and records every candidate `not_judged` under `authority: "off"`. With authority on it judges under the budget (`tap_grid.falsifiers.falsify_candidates(budget=…)`: candidates beyond it are `UNDETERMINED(budget)`, one warning names the shortfall, the record carries `budget: {limit, used, left}`) and applies each verdict through the write pipeline — judging, every write and the record in **one transaction**, each target's spine row locked before the fence is read so the fence and the write see one committed state — `DROPPED_FROM_OBSERVATION` → `delete_node(reason=<candidate reason>, cascade="contained")`, `RELOCATED(renamed)` → the name patch, `RELOCATED(transferred)` → `delete_edge` on the parent's containment edge only — each behind **the fence**: no create/update `BatchEvent` on the target from a committed batch outside this run's produced set after the candidate record's `recorded_at`; `Entity.version` is never consulted. A transfer is fenced on the edge it ends as well as on the node: an edge re-created since the record was derived carries a `link` event on the edge and no update on the child, and it rejects the verdict too. **The delete licence is bound to the row:** `grid.reconcile` stands in for `grid.delete` only inside `tap_grid.write_guard.reconcile_write_scope`, which the verb opens around each write for the rows the verdict entry names — the candidate, and for a transfer the parent's containment edge — derived from the entry and never from the operation; the backstop compares the delete ops' targets to that set, so an operation formed for the wrong row is refused rather than licensed by its own target, and a defect in the verb cannot widen a verdict past the row it judged. The scope is a contextvar, not a capability: in-process code could open it, as it could `unguarded_write`, so the collectors-never-retire walk forbids naming either — a lint-guarded trust boundary, stated as such. A tombstone is also fenced against a **contradiction** (Issue# 656 - tap, `-9`): before the write, the target's contained closure — `tap_grid.services.contained_closure_locked`, the cascade's own discovery, locked and re-discovered under the locks until stable, gated by `grid.reconcile` because taking locks is not a read — is checked for a node observed since the record was derived by any batch that has not failed (an open batch may commit a moment later); one found, or a closure over the cap, refuses the tombstone as `contradicted` with the descendants named. Every entry records `applied: {write, outcome ∈ applied | rejected_stale | refused | not_applicable | contradicted, error}` and the record sums them. Resolution atomicity (-5) is the identity gate's (`resolve_identity`, `req-grid-import-grift-identity`); the verb never mints. **Not built here:** scoped authority (Future), edge re-derivation (`req-grid-reconcile-falsifier-5`), the bulk-absence breaker (`req-grid-reconcile-breaker`, Backlog).
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-grid-reconcile-verb-1 | Collectors never retire | Implemented | No collector code path calls a delete or purge verb; a test walks collector modules and fails if one does. | Mechanical, CI: `tap_cares/tests/test_collectors_never_retire.py` walks `tap_cares/collectors/**` and every installed plugin's `collectors/` package by AST — delete verbs, `write_batch`, `reconcile`, the lock-taking `contained_closure_locked`, the internal bypasses, the write-guard scopes (`reconcile_write_scope`, `unguarded_write`, `service_write_scope`) and the run-configuration key — with a positive control and a coverage check. |
| req-grid-reconcile-verb-2 | Off by default | Implemented | With no reconciliation authority configured, a run retires nothing and says so. | `test_reconcile_verb.py::TestAuthorityOff` (no probe runs, every entry `not_judged`, `authority: "off"`); `test_completeness_flow.py::test_the_run_ends_with_a_verdict_record_that_says_authority_is_off` end to end through `run_collection`. |
| req-grid-reconcile-verb-3 | Budget bounds the pass | Implemented | With the budget exhausted, remaining candidates are `UNDETERMINED(budget)`, the run succeeds, and the shortfall is recorded. | `TestBudget`: budget 5 over 15 candidates → 5 judged and applied, 10 `UNDETERMINED(budget)`, `budget: {5, 5, 10}`, warning `[edf1]`. The budget counts candidates handed to falsifiers, since batch is the interface. |
| req-grid-reconcile-verb-4 | Stale verdicts rejected | Implemented | A verdict from an older generation applied after a newer successful observation is rejected, including when `Entity.version` did not change. | Review acceptance case. `TestTheFence`: a re-observation after derivation → `rejected_stale`; the same with the version bump undone at the row; this run's own batches never fence its verdicts; a transfer whose ownership edge was re-created since derivation → `rejected_stale` on the edge's own `link` event, the fresh edge live. |
| req-grid-reconcile-verb-5 | Resolution is atomic | Implemented | Two concurrent resolutions of the same absent key yield one live identity in the intended scope, or a recorded conflict; no edge is left pointing at a losing identity. | Review acceptance case. Identity resolution is the gate's (`resolve_identity` under the advisory lock, `test_grift_identity.py::TestTwoWriters`); the verb never mints. `TestAtomicity`: two concurrent reconciles of one candidate → one tombstone, one pipeline noop, no live edge onto it. |
| req-grid-reconcile-verb-6 | Ordering enforced | Implemented | There is no public per-tier entry point; the walk is outer-in within the verb. | `test_there_is_one_public_entry_point`: `reconcile` is the only reconciliation name `tap_grid.services` exports; the walk is the candidate record's surface order, derived outer-in. |
| req-grid-reconcile-verb-7 | Unknown scope withdraws nothing | Implemented | A run whose scope enumeration is unknown produces no `scope_withdrawn` retirement. | Review acceptance case: selection shrinks. `TestWithdrawal::test_an_unknown_previous_scope_withdraws_nothing` (no previous run offered → no withdrawn candidate → nothing retired, whatever the falsifier says). |
| req-grid-reconcile-verb-9 | Contradiction fenced at apply | Implemented | A tombstone whose contained closure, read under locks at apply, holds a node this run observed live (the derivation invariant re-asserted — a re-parent leaves only a `link` event) or a node observed since the candidate record was derived — by any batch that has not failed, an open one included — is `contradicted` at apply; the closure is read as the cascade reads it before writing (locked, re-discovered under the locks until stable; `tap_grid.services.contained_closure_locked`, gated by `grid.reconcile`), nothing is written and the record names the descendants. | Issue# 656 - tap. `TestContradiction`: a descendant observed after derivation; one attached between the first look and the locks; one observed in a still-open batch; the locked read denied to a `grid.read` actor. `applied.contradicted` sums them; log `[2611]`. |
| req-grid-reconcile-verb-8 | Withdrawal is not privileged | Implemented | A `scope_withdrawn` retirement passes the same authority, freshness, cascade and audit path as any other. | `TestWithdrawal::test_a_withdrawal_retires_through_the_same_path`: the withdrawn children are tombstoned with reason `scope_withdrawn` and the same `reconcile` audit metadata as a dropped candidate. |
