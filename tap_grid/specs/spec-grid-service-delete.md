...
| req-grid-service-delete-cascade | [Contained-Subtree Cascade](#contained-subtree-cascade) | Proposed | `cascade="contained"` walks the model's `CONTAINMENT_EDGES` recursively in one transaction, capped and cycle-safe; a refusal on any child rolls it back. Preflight, `cascade_force` and integrity findings are Backlog (down-scoped 2026-09-17) |
...
| req-grid-service-delete-cascade-2 | Undeclared is not followed | Proposed | A relation absent from `CONTAINMENT_EDGES` is not traversed; cascade uses its own evaluator over that declaration, never the edge-permission union. | |
...
| req-grid-service-delete-cascade-4 | Cancelled on refusal | Proposed | Lacking authority over any child in the walk, the cascade rolls back and writes nothing — including the target. | The blocking-types report is `-8`'s Backlog half. |
...
| req-grid-service-delete-cascade-8 | Every ended edge is authorized | Backlog | Authority is checked over each edge the cascade ends, including reference edges whose far endpoint lies outside the contained subtree, and a refusal names the blocking types. | Make-it-right. |
...
| req-grid-service-delete-cascade-11 | Bounded work | Proposed | A walk that would retire more than `TAP_CASCADE_MAX_CLOSURE` nodes raises `cascade_closure_too_large` and, the block being atomic, writes nothing. | The retrieval bound (`LIMIT remaining + 1`, `EXPLAIN`) is `-15`. |
| req-grid-service-delete-cascade-12 | Declared on the model, not on permissions | Proposed | `CONTAINMENT_EDGES` is a dedicated declaration; a guard fails if it names an edge type the model's `OUTBOUND_EDGES` does not permit; `OUTBOUND_EDGES` carries no cascade semantics. | Ruled 2026-09-17. |
| req-grid-service-delete-cascade-13 | Cycle-safe | Proposed | A cycle in the declared containment graph terminates the walk without error; each node is retired once. | |
| req-grid-service-delete-cascade-14 | Provenance names the parent | Proposed | Every node and edge retired by the walk records the run, scope, reason and the parent it was a consequence of in `BatchEvent.metadata`; an imperative GRIFT delete and a service call produce the same records. | |
