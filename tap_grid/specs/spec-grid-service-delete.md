...
| req-grid-service-delete-cascade-plan-3 | Stale plan refused | Proposed | Any of the following between plan and apply makes `delete_node(..., expected_plan=)` write nothing and return `cascade_plan_stale`: a child added, reparented, or version-bumped; **a reference edge added to or removed from a closure node**; or a containment edge added or removed. | Edge cases added on review (Codex, round 6): endpoint versions do not move when an edge is created. |
