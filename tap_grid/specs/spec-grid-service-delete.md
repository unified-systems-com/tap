...
| req-grid-service-delete-tombstone-7 | No Live Edge Onto A Tombstone | Implemented | At no committed state does a live edge have a tombstoned endpoint. The delete side: a retired node's incident edges are ended with it (`-2`, the cascade). The create side: edge creation row-locks both endpoints in the delete's global order and refuses a tombstoned one as `entity_tombstoned`, so a delete of an endpoint either waits for the edge to commit and then ends it, or has committed and is seen as a tombstone; the importer treats a tombstoned endpoint as dangling (`req-grid-import-grift-dangling-1`). The write side: every mutating verb locks its target row before deciding on `deleted_at`, so a row tombstoned while a replace waited is refused, never overwritten. `Edge.live_onto_tombstones()` is the invariant's read, empty by construction, checked by both corpora after every scenario. | Ruled by George 2026-09-18 (Issue# 609 - tap; the replace side Issue# 611 - tap, sibling of the delete-side race Issue# 590). `tap_grid/services/_impl.py` (`create_edge` branch; the single locked target load), `tap_grid/grift/importer.py` (dangling check on live rows); `tap_grid/tests/test_edge_onto_tombstone.py` (service refusal, importer both modes, both contention orders, the replace race, the guard's own control), `tap_grid/tests/test_batch_corpus_concurrency.py::TestTiming` (the two formerly pending cases). |
...
**Known limitation — ownership is a directed graph (ruled 2026-09-19).** The walk reads one declaration and follows edges from a parent; a child that a second live parent outside the closure still contains is retired with the first parent that cascades (the `loops` and `ownership` corpus families pin it), and the cascade logs a warning naming the outside parent (`[341b]`, Issue# 650 - tap). Nothing keeps a shared child alive today. When a real relationship needs that, or any belonging a single directed edge cannot express, the answer is deterministic per-type ownership and deletion logic with `CONTAINMENT_EDGES` as the declarative default — Issue# 649 - tap, held until a case exists.
...
| req-grid-service-delete-cascade-6 | Closure before ending edges | Implemented | The walk discovers the whole closure, locks every node of it in id order, recomputes the closure under those locks, then locks every edge it will end — before writing anything. | `_discover_closure`, `_closure_under_locks`, `_lock_rows` in `tap_grid/services/_impl.py`; `test_delete_concurrency.py`. Arrived early for correctness, not preflight: it is what removes the lock-order inversion of Issue# 590 - tap. |
| req-grid-service-delete-cascade-7 | Reparenting race | Implemented | A child whose containing edge was removed between discovery and lock is not retired; a child attached in that window is. The closure is recomputed under the locks, and a locked row cannot gain an edge, so the recomputation is exhaustive. | `test_delete_concurrency.py::TestClosureUnderLocks`. The phase-5 freshness fence is a different question (stale verdicts), not this race. |
...
| req-grid-service-delete-cascade-19 | Overlapping writers never deadlock or double-write | Implemented | Every delete takes its row locks in one global order — the nodes it will retire ascending by id, then the edges it will end ascending by id — so two overlapping deletes serialise instead of deadlocking; the endpoint tombstone is conditional on the row still being live and records an event only for the rows it claimed, so an edge another writer ended first is neither bumped nor recorded again; a database deadlock, should one still occur against a writer that is not a delete, surfaces as the typed, retryable `write_conflict` with nothing written. | `_lock_rows`; `is_deadlock` (`tap_grid/exceptions.py`) and `write_batch`; `test_delete_concurrency.py` (same root twice, a child deleted mid-cascade, two cascades at a shared child, each run several times), `test_write_conflict_is_typed`. Issue# 590 - tap, found by the cascade confirmation corpus. |
