...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-grid-edge-schema-required-1 | Non-Empty Properties Demand A Schema | Partial | Writing non-empty `properties` to an edge type with no registered schema raises a fail-closed error at the service layer and `Edge.save()`. | Narrows `req-grid-edge-properties-6/-7`. **Warn mode Implemented 2026-08-11** (`constraints.ENFORCE_EDGE_SCHEMA_REQUIRED = False`, log site `[d393]`) — the 0.1.x posture per the published contract. **Flip retargeted to 0.3.0 (ruled 2026-09-19, Issue# 660 - tap):** 0.2.0 was cut on the natural-key breaking change (UUIDv8 over SHA-256) before the `-3` burn-down was done, so 0.2.0 still warns; the 0.3.0 flip sets it True and completes this ACID. |
