...
| req-grid-batch-corpus-format-6 | The second pass's issue shapes are scenarios | Implemented | Issues# 606, 607, 608, 351, 609 and 322 - tap each have a classified scenario: the fixed ones expect the fix; the open ones are verified-pending in their exact wrong shape; #322 is pinned as today's contract with the open question named. | `collision`, `atomic`, `provenance`, `hotlink`, `reload`, `replay` families; `test_the_second_pass_issue_shapes_are_scenarios`. |
...
**Three truths (second pass).** The persisted truth is read beyond liveness and version: a row's `Entity.dimensions` when the scenario declared them, every edge's `from`/`to` against the resolved ids of the names it was declared between, typed-row fields through `get_node`, and the `batch_id` stamped on the typed row against the batch the model says last wrote it. The API report is read for counts where a scenario spot-checks them. Provenance is read as the batch of every event recorded during the run, which must be a batch this run committed. Generated ids are symbolic: a minted ref id is checked to be a UUID version 7 distinct from every non-alias name and nothing else; a failed batch's `resolved_refs` are never read as proof of a row. The caller's document is deep-copied before the call and compared after it. After every scenario the tombstone invariant is read: no live edge has a tombstoned endpoint.
...
| req-grid-batch-corpus-runner-5 | Three truths, symbolic ids | Implemented | Persisted state (dimensions, endpoints, typed fields, typed batch stamp), the API report (counts) and provenance (event attribution) are each compared; minted ids are UUIDv7 and distinct, never compared by value; the caller's document is unchanged; the checker rejects a corruption on every layer. | `runner._check_row`, `_check_symbolic_ids`, `event_batches_since`; `test_batch_corpus_checker.py::TestTheSecondPassRejects` (ten controls). |
...
**Schedules (second pass; George, 2026-09-18).** `TestSchedules` runs the write × delete permutations as isolation-spec-style schedules, each docstring stating the acceptable serial outcomes: write/write on one id (two replaces, version 3, the contender's content); write then delete (the tombstone is the last write); a delete section then an upsert of the same id in two batches (the upsert is refused — pending Issue# 611 - tap in its exact shape); delete/delete (tombstoned exactly once; the contender's bundle-reason event is the one thing the schedule leaves open, recorded); edge create against endpoint delete in both orders (delete first: the edge is refused — pending Issue# 609 - tap, recognised as the live edge onto the tombstone and cleared; edge first: the delete ends the new edge); a cascade during a subtree write (refused — pending #611). Ref-write against id-delete and two batches creating one ref'd object are the first pass's cases. `contend(must_block=False)` is the schedule for a contender whose lock relationship is itself under test. After both writers finish every schedule asserts both invariants: `live_edges_onto_tombstones()` is empty and `rewritten_tombstones(ids)` is empty (no update/link event after a row's delete/unlink event); the first is also an autouse fixture over the whole module. PR# 624 - tap gives the first invariant a home on `Edge.live_onto_tombstones()`; the harness function collapses into it when both are on main.
