...
| req-grid-batch-corpus-runner-5 | Three truths, symbolic ids | Implemented | Persisted state (dimensions, endpoints, typed fields, typed batch stamp), the API report (counts) and provenance (event attribution) are each compared; minted ids are UUIDv7 and distinct, never compared by value; the caller's document is unchanged; the checker rejects a corruption on every layer. | `runner._check_row`, `_check_stamp`, `_check_symbolic_ids`, `event_batches_since`; `test_batch_corpus_checker.py::TestTheSecondPassRejects` (twelve controls: a wrongly stamped live edge and a tombstone among them, Codex on PR# 637 - tap). |
