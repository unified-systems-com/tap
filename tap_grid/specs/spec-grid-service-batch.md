...
| req-grid-service-batch-metadata-8 | Name Clamped To Both Ends | Implemented | `create_batch()` clamps `name` to the shortest `max_length` among `Batch.name` and `Entity.name`, read off the fields. A caller composing a name from data it does not control cannot overflow either column. | `clamp_to_fields`. A batch opened inside a caller's transaction would otherwise take that transaction down when the name overflowed. |
