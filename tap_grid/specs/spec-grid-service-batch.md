...
Names are also **clamped at that same chokepoint**, to the shortest `max_length` among `Batch.name` and `Entity.name` (read off the fields, so widening a column cannot leave a stale constant behind). A caller composing a name from data it does not control — a schedule name, a plugin slug, a bundle path, each of them 255 itself — cannot know the budget left after its own prefix, and a batch is frequently opened *inside* the caller's transaction, so an overlong name does not merely lose a label: it rolls that caller's work back. A truncated display name is the far smaller loss. Because the clamp lives at the one place that writes both ends, no caller carries a second copy of the limit.
...
- **Joining an existing batch keeps that batch's name.** A `batch_name` or `batch_description` that equals the existing batch's value is accepted; one that differs is refused with `ValueError` before any operation runs. The check runs inside `write_batch`'s transaction, after `_ensure_batch` and before any operation, against whatever `Batch` row the write will land in, under a row lock. The caller's own freshly minted batch matches by construction (the name compared as `create_batch` clamped it), and a batch another writer created first is refused. If two writes race to mint the same new `batch_id`, the loser's create fails inside the transaction and the whole write fails closed as an `internal_error` with nothing written, rather than being refused with `ValueError`; either way it cannot land in the other writer's batch under the wrong label. Two callers minting one caller-chosen id at the same instant is a caller bug, and the service layer does not serialize it. The error names only the caller's value, never the stored name or description, so probing a guessed `batch_id` discloses nothing. The rule applies to the batch scope in force however it arrived: an explicit `caller_context.batch_id`, or an ambient one bound at a boundary (a program actor's scope, the test harness's per-test batch). Request handling binds no batch, so an ordinary ad-hoc write mints its own. The alternatives were worse: silently ignoring the argument lets a caller believe its label landed, and applying it would relabel every write already in that batch after the fact.
...
- **Callers.**
  - The web UI binds a default derived from the edit on the request's context for the duration of a save — `"Web edit: <subject>"` / `"Edited in the web UI by <user> via the <surface>."` — around the panel editor, the panel JSON editor, the generic object editor, and a panel type's POST handler dispatched from the panel fragment endpoint (the editor panel saves through a registered editor's `handle_save` there). The request middleware binds a fresh context with no batch and no label at request entry (`CallerContextMiddleware`), and the default is bound after form validation and immediately around the save, so no write in the request precedes it. Because it rides on the context, it also labels the service writes a plugin's own editor or panel type makes in its `handle_save`, with no plugin change. There is deliberately no visible field yet: the affordance waits until enough edits come through the UI to design it.
  ...
  - **The harness default cannot stand in for a production caller.** It has a cost: a production caller exercised by a test inherits the harness's batch id and label, so it could pass there and be refused in production, where nothing binds either. The harness therefore audits every write that relies on its scope — the name or the description it would mint under is the harness's (each judged on its own, so supplying one and inheriting the other is caught), and it lands in the harness batch or mints a new one — tracing it to its first caller outside the service layer. If that caller is production code, the test fails naming the file and line. Nothing is listed by name: the service layer's modules are read off the service functions, and "production" is any module under the pytest rootdir that is not a test module and not under an `--ignore`d path, so each repository audits its own code and a plugin's code is audited by the plugin's own test run.
...
| req-grid-service-batch-label-required-7 | The Harness Scope Cannot Stand In For Production | Implemented | Under the test harness's default label, a write whose first caller outside the service layer is production code under the rootdir, and which relies on the harness batch or label, fails the test naming the call site; test code, a labelled production write, and code outside the rootdir are not flagged. | `_HarnessLabelAudit` in `tap/pytest_harness.py`. |
