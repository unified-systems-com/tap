...
**The placeholder column.** `Entity.natural_key` is text, nullable, non-unique, plainly indexed, and stays in `Entity.SPINE_EXCLUDED` so the Gryphon envelope and the gridkin corpus are untouched. Its named future — recorded so it is not re-derived — is a **per-type composed, readable identifier** (`pkg:npm/%40scope/name`, `arn:aws:iam::123456789012:role/x`, `github:repository:12345`): searchable across types by equality or prefix, and the column on which a live-uniqueness constraint `(entity_type, natural_key, identity_scope) WHERE deleted_at IS NULL` becomes expressible on the spine when perspectives arrive. When it is filled it is **derived by declaration, never authored** — the pattern `Entity.name` already follows (`req-grid-node-display`). Until then it is inert, and inert is the honest state: a half-populated column would imply a second way to find a row.
...
| req-grid-entity-natural-key-7 | Placeholder stays off the surfaces | Proposed | `natural_key` remains in `SPINE_EXCLUDED` until a consumer exists; the GRIFT import envelope is unchanged and a test asserts a supplied `natural_key` is rejected. | |
...
| req-grid-entity-natural-key-9 | Resolution is deterministic | Backlog | At the gate: `resolve_identity` mints on zero matches, returns on exactly one, and raises `AmbiguousIdentity` on more than one — never silently selecting a row. | Gate item; either shape. |
| req-grid-entity-natural-key-10 | Search ignores dimensions | Proposed | `find_existing` matches the declared fields among live rows only; no dimension participates. A node whose dimensions differ between two runs of one collector still finds its existing row. | The account-minted-per-repository case. |
| req-grid-entity-natural-key-11 | Placeholder column is inert | Proposed | `Entity.natural_key` is text; no code path outside models, migrations and the spine field inventory references it; its help text names it a placeholder and names the trigger. | |
| req-grid-entity-natural-key-12 | Search generated from the declaration | Proposed | Every keyed type has `find_existing` and an index over its declared fields (asserted by introspection); a `KEYLESS` type has neither; on a fixture with one, none and two live rows sharing declared values it returns the row, nothing, and raises. | |
| req-grid-entity-natural-key-13 | Resolution fails closed | Backlog | At the gate: ambiguity fails the whole batch; the detail is recorded on the `Batch`; one application-class `Flaw` is emitted per failed batch; resolution and creation share one transaction under a lock. | Gate item; either shape. |
