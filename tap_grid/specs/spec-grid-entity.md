...
**Normalization, then canonicalization.** RFC 8785 (JCS) fixes serialization, not semantics, so each recipe also declares: string versus number for identifiers (source ids that exceed IEEE-754 safe integers are strings), absent versus null, case folding, array ordering and Unicode form. And a **recipe version**, so redefining a key document is a recorded migration rather than silent churn.
...
- The GRIFT entity envelope is `additionalProperties: false`, so transporting a peer's key is a schema change. Whether keys are **transported or re-derived at import**, and how recipe versions travel, is an open decision.
