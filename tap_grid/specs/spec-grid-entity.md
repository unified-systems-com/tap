...
**Normalization, then canonicalization.** RFC 8785 (JCS) fixes serialization, not semantics, so each recipe also declares: string versus number for identifiers (source ids that exceed IEEE-754 safe integers are strings), absent versus null, case folding, array ordering and Unicode form. And a **recipe version** — a class attribute on the model beside the key document itself, never a column on `Entity`. The version describes the *recipe*, which is a property of the type, so storing it per row would be the same fact written twice, on every row, forever. It is also not `Entity.version`, which counts row mutations for concurrency: conflating "this row changed" with "the recipe changed" is the one-field-two-meanings defect. Redefining a key document is therefore a **Django migration** — the migration is the record, with a name, a number and an ordering already in the tree — and the backfill's check mode is what detects a recipe edited without one, by recomputing and comparing rather than by trusting a stored marker.
...
| req-grid-entity-natural-key-7 | Spine surface updated atomically | Proposed | `SPINE_FIELD_NAMES`, its drift-guard test and the GRIFT envelope schema change in the same commit as the field. | Mechanical, CI. |
| req-grid-entity-natural-key-8 | Recipe version recorded | Proposed | A key document carries a recipe version, and a change to a recipe is a recorded migration rather than an in-place recomputation. | |
