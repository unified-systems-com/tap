...
*Slice 1 of shape A landed 2026-09-18 (Issue# 593 - tap): the envelope half — `entity_id` XOR `ref`, endpoint refs, one resolution pass before preflight, the `ref → id` map on the result — with a mint-only resolver; `req-grid-import-grift-identity-3`. The verb (Issue# 594 - tap) and this section's rewrite (Issue# 595 - tap) follow.*
