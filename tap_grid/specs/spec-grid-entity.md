...
**The key is a UUIDv8 over SHA-256 — RULED 2026-09-16.** `uuid8(SHA-256(namespace bytes ‖ canonical document))`, the digest sliced octets 0-5 (48 bits), 6-7 (12 bits used) and 8-15 (62 bits used); `uuid.uuid8` sets the version and variant. **That slicing is canon** — change which bytes go where and every key ever produced moves. The namespace is a literal, `f83d127a-4ca8-5441-b656-446eb0be3f29`, whose value is what `uuid5(NAMESPACE_DNS, "natural-key.tap")` produced; recording it rather than recomputing it keeps the provenance without executing SHA-1 at import. It is mixed into the hashed input explicitly, because v8 does not do that for you.

UUIDv5 is the natural fit — name-based and deterministic — and was the original choice. It is SHA-1. SHA-1 still executes under the FIPS provider, because it remains an approved *hash* under FIPS 140-3 (what lapses is its use in signatures), but NIST is retiring it for federal use by 2030 and this repository ships FedRAMP tooling, so SHA-1 in the identity layer is a crypto-BOM conversation waiting to happen. v8 is RFC 9562's slot for a vendor-defined layout, which is what this is. Moved while **zero keys were stored anywhere**: the change cost nothing then and would cost "rewrite every key in every grid" after the first backfill. The cost accepted in exchange is that a v8 does not describe itself the way a v5 does — hence the layout above being documented rather than implicit. There is no UUIDv9; RFC 9562 defines versions 1-8.

**Normalization, then canonicalization — RULED 2026-09-15.** Canonicalization is `sort_keys` plus compact separators plus `ensure_ascii=False` (`tap_grid/natural_key.py`, matching the in-house precedent at `tap/boot_records.py`), **not** an RFC 8785 implementation. JCS and that form differ in exactly two places: number serialization, where JCS mandates ECMAScript `Number::toString`; and object-key ordering for characters outside the BMP, where JCS orders by UTF-16 code unit and Python by code point. The key document forbids the inputs that could reach either — **values are `str` or `int` only** (no floats, and `bool` is refused explicitly because it is an `int` subclass in Python), and **property names are ASCII** — so the cheap form is *provably* equivalent to JCS over the permitted domain rather than assumed equivalent over one that is not permitted. Taking a JCS dependency to cover cases the domain rejects anyway would be cost without coverage.

Normalization still has to be declared per recipe, because canonicalization fixes serialization and not semantics: identifiers are strings wherever a source id could exceed IEEE-754 safe integers, absent versus null, case folding, array ordering and Unicode form. An absent constituting value yields **no key** (null) rather than a key derived from a hole — a data condition, not an error. A value outside the permitted domain raises, because that is a bug.

If a key document ever genuinely needs a value outside the domain, the escape hatch is a **versioned key derivation** (`tap#475`), never a quiet widening: changing what the canonicalizer accepts changes every key it has ever produced. **There is no separate recipe version — RULED 2026-09-15.** A key document is part of the model definition, so a recipe change is a *model* change and needs no version of its own. Neither a column on `Entity` nor a class attribute: either would become a second counter the moment BaseModel classes are versioned, and two version numbers for one fact is the derive-twice defect. It is also not `Entity.version`, which counts row mutations for concurrency — conflating "this row changed" with "the definition changed" is the one-field-two-meanings defect.
