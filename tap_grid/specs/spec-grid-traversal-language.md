...
**Ruled 2026-09-11 (tap#433): union is the intended semantics.** Two facts decided it.
The only production multi-`MATCH` query in the fleet — git-serious's org picture, six
per-label `MATCH`es onto one envelope — wants exactly a union; under Cypher it is a
six-way cross product that goes blank whenever any label has zero rows. And composition
alone would unblock none of the audit's candidate queries: the seven that need a join
all also need variable-length paths (tap#259).

What made union dangerous was never the union — it was that the Cypher reader's join
was *typeable* and answered silently. So the ruling comes with a fence
(`req-grid-traversal-lang-shape-8`): a variable bound in more than one mandatory `MATCH`
clause is rejected above the dispatch fork, naming the variable, both clauses, and the
remedy. Disjoint clauses union as before; the ambiguous spelling no longer runs.
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-grid-traversal-lang-filters-1 | Inline Property Maps Supported | Implemented | Node and edge patterns may include inline property filters; values are AND'd into the queryset for the bound pattern. A node map `{k: v}` is exactly `WHERE var.data.k = v` — same allowlist, same type strictness, byte-identical SQL. A labelless node with a map is rejected (no model to resolve against). | HISTORY, recorded per #196: this row read "Implemented" for both halves from 2026-06 to 2026-08-31 while only the edge half existed — node maps were parsed and silently dropped, so 43 of 71 BloodHound-corpus queries returned unfiltered supersets. Worse, the Notes cited a function (`_apply_inline_edge_property_filters`) that never existed in the codebase and a test file (`test_gryphon_inline_edge_filter.py`) that never existed on any branch: the row was written as the work was *intended* to land, and nothing checked that its citations resolved. Real anchors — edge half: the inline edge-property filter inside `_build_chain_queryset` (`executor.py`), tests `test_inline_edge_property_filter_*` in `test_gryphon.py`; node half (2026-08-31): `_node_inline_prop_filters` applied at the type-scan, chain, bare-scan (reject) and OPTIONAL MATCH sites, pinned by `tap_grid/tests/test_gryphon_construct_effect.py`, which mechanically forbids the whole accept-and-drop class. |
...
Accepted by the owner on 2026-09-24. The capability is implemented in `tap_grid/gryphon/` (grammar `param_test`, the `ParamNullTest` leaf, and `_fold_ast_param_predicates` on the execute chokepoint) and covered by `TestGryphonParamNullParser`, `TestGryphonParamNullExecutor` and `TestParamNullFoldCoversEveryWhereSlot` in `tap_grid/tests/test_gryphon.py`.

**The route here is recorded rather than tidied away, because the next person to add a capability should see it.** This requirement did not pass through `Approved for Development`, which in the TAP status vocabulary precedes development. `build-gryphon-capability` requires an independent design review and the owner's explicit sign-off BEFORE any code; neither happened. It was built inside a defect-fixing task whose brief said both "follow the skill" and "fix it" and never said the gates bind, and the agent that wrote it flagged its own commit as a proposal rather than gate-cleared.

It then reached `main` in `PR# 783 - tap` alongside two unrelated defect fixes. It was carried as `In Development` from that point — the weakest claim that was true, so that nothing in the repository asserted an approval that had not been given — until the owner ruled to keep it. That ruling is what this status now records; it is an acceptance after the fact, not a gate that was cleared.

The standing procedure that exists to prevent the same route is `tap_grid/skills/gryphon-defect-response/SKILL.md`: a defect is fixed on the fly, a missing capability stops and returns to the owner.
