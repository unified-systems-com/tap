...
does, not what the language has committed to. The join itself is carried as
`req-grid-traversal-lang-shape-7` (`Proposed`, not built) so the capability gap stays
visible where statuses are scanned, rather than only in this prose.
