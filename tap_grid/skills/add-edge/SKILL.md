...
- **A closed endpoint list naming another plugin's types.** The mirror of the mistake above, and the more damaging one. `targets: ["aws_core__aws_route53_zone", "aws_core__aws_region", "aws_core__aws_cloudfront_distribution"]` inside github_core (real, `origin/main` 2026-09-14) is an undeclared cross-plugin dependency that no guard catches, makes a fourth AWS type an edit to github_core, and silently breaks if the peer plugin is not installed. Prefer a substrate type both sides can target; failing that, omit the side. See "Cross-plugin endpoints" in Step 2.
