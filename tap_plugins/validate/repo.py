...
def _check_workflows(repo_root: Path, result: ValidationResult) -> None:
    ...
    who receives a nightly failure the plugin author cannot fix is still unruled (``tap#367``),
    and a lane whose red nobody owns is not something to force on a repository.
    """
    ...
    for rel, missing in (
        ...
        if missing == "fail":
            ...
            check.warn(
                f"no {rel} — nothing probes core `main`, so the upper bound of this plugin's "
                "declared requires_tap range is never tested and can only be found by an "
                "adopter (plugin standard C2)",
                path=rel,
            )
