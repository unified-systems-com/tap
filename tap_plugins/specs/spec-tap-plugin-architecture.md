...
| req-tap-plugin-arch-identity-4 | Repo Decoupled | Implemented | Repo name is convention-only, not load-bearing; identity survives standalone↔monorepo moves. Repo-path-as-identity rejected. | Exercised: every plugin is a standalone repository since the 2026-09 eviction (`zizmor-tap`, `git-serious-tap`, `git-core-tap`, `dcom-tap`), the convention being the dist name; owner and host are the plugin owner's choice, never assumed by the scaffold (tap#452) |
...
| req-tap-plugin-arch-sources-7 | Wheelhouse Digest Pin | Implemented | A `wheelhouse` source MAY declare `sha256` for its wheel; pre-boot hashes the resolved `.whl` and **aborts** on a mismatch before any install runs. Absent ⇒ warn and proceed (report-only, as the DCO gate does), so existing records keep booting. A missing wheel is NOT this check's error — it defers to `uv --no-index` so not-found has one path, not two. | `tap/preboot.py::_verify_wheelhouse_digest`. Closes the content half of `-6`'s deferred edge: the `git` arm is content-addressed by its rev, so the **offline** arm — the one an airgapped operator relies on — was the weaker of the two, which is backwards. Signing remains deferred to `-versioning-5`. |
