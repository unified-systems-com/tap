...
TAP Core Architectural Rules
    ...
    Entity ids are ASSIGNED (UUIDv7 at first sight), never derived from a hash of the thing's facts: a derived id cannot coexist with a terminal tombstone (req-grid-entity-natural-key). Every model declares how one of its rows is found again — `NATURAL_KEY` naming stable source identifiers, or `KEYLESS` with a reason — and the search is generated from that declaration; undeclared is a guard failure. `Entity.natural_key` is a text placeholder nothing reads or writes until its named consumer arrives. Containment for cascade is a dedicated `CONTAINMENT_EDGES` declaration on the model (req-grid-service-delete-cascade); `OUTBOUND_EDGES` is edge permission only.
