...
| req-tap-auth-github-oauth | [GitHub OAuth Provider](#github-oauth-provider) | Implemented | Second provider type; plain OAuth 2.0 (GitHub is not an OIDC IdP for user login); identity keyed on the numeric user id; allowed_user_ids/allowed_logins/owner_only policy; live credential-adjudication self-test |
...
- **Identity is the GitHub numeric user id, never the login.** A login is renameable by its owner and, once released, registrable by someone else, so keying identity on it would make a rename read as a new person and a recycled handle inherit a stranger's account and grants. allauth's `GitHubProvider.extract_uid` already returns `str(data["id"])`, so the durable subject reaches `ExternalIdentity.(provider_id, subject)` unaided. `allowed_logins` is an authorization filter re-checked on every login — the same standing as `google_oidc`'s `allowed_emails` — and never an identity key.
- **The access policy lives in `evaluate_access`**, the existing per-provider seam, not a second gate. Its clauses are a union; at least one must be configured:
  - `allowed_user_ids` — numeric ids. The rename-proof form, preferred when the account is known.
  - `allowed_logins` — logins, matched case-insensitively.
  - `owner_only` — "only the account that created this instance". The intent is declared in the profile; the VALUE is derived at boot from the environment (`TAP_AUTH_INSTANCE_OWNER`), so committed config never hardcodes a handle. This is the shape the single-operator Codespaces standup needs.
...
| req-tap-auth-github-oauth-3 | Access Policy Required | Implemented | A `github_oauth` provider must configure at least one of `allowed_user_ids` / `allowed_logins` / `owner_only`; an unset policy FAILs the offline self-test. There is no any-GitHub-account login. | |
...
| req-tap-auth-github-oauth-5 | Owner-Only Derived | Implemented | `owner_only` declares intent in the profile while the owner's value is derived from the environment; an unresolvable owner denies with `policy_unresolvable` and FAILs the offline self-test, never degrading to allow-anyone. | |
