...
- **A name carries exactly one ruling.** A second pattern arriving under a name already ruled on is a route nobody has ruled on wearing a ruling written for a different one — at a `SERVE` name, a new view served by inheritance. The first occurrence keeps its verdict; the repeat is closed regardless of the table, and the inventory test asserts no two mounted routes share a name (the precondition the table rests on).
