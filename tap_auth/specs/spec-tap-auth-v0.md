...
- Provider secrets are resolved from the shared `*.secret.json` store via the **app-neutral `tap/runtime_secrets` resolver** (the same file-discovery and envelope contract tap_cares uses; see `spec-tap-cares-secrets` → *Shared Resolver*). tap_auth deliberately resolves **independently of the `tap_cares` app**: allauth settings are built at settings-import time, before `tap_cares.ready()` loads its registry, and tap_auth must not depend on tap_cares (the `tap_*` apps stay independently shippable; `tap/` and `tap_grid` are the only shared-dependency layers). tap_auth therefore reads the upstream resolver directly and owns its provider-side OAuth-client data-block schema (`tap_auth/schemas/oauth_client_secret.schema.json`, envelope `kind` `oidc_client` or `oauth2_client` — one schema and one resolver for every provider type, because an OIDC client credential and a plain OAuth 2.0 one carry the same two fields and the kind names which protocol issued it); the tap_cares *registry*, resilient-load report, and secrets health probe are not on this path.
...
----
RID: `req-tap-auth-github-oauth`

Status: `Implemented`

`github_oauth` is the second concrete external provider type, and the one that proved the provider registry is a registry.

#### Implementation

- **GitHub is not an OIDC provider for user login.** `https://github.com/.well-known/openid-configuration` returns 404 (checked 2026-09-20); the issuer that does answer, `token.actions.githubusercontent.com`, is Actions *workload* identity, not human sign-in. GitHub user auth is plain OAuth 2.0.
- `github_oauth` therefore rides **allauth's `github` provider**, not `openid_connect`. This is what made the provider abstraction's real shape visible: it is "produce one allauth APPS entry", not "be OIDC". Three places in the framework had encoded the narrower reading while `google_oidc` was the only implementation, and each is now a fact the provider states about itself:
  - the allauth-settings wiring grouped every APPS entry under a module constant `openid_connect`; providers now declare `allauth_provider`. A GitHub entry filed under the OIDC engine would have raised `KeyError: 'server_url'` on the first login — a runtime failure produced by the wiring, not by the operator.
  - the callback URL was derived as `/auth/oidc/<provider_id>/login/callback/`; `callback_url` is now per-provider, because allauth routes openid_connect apps *per `provider_id`* and OAuth2 apps at one fixed path per provider class.
  - display-profile extraction in the social adapter read literal Google claim names (`picture`, `given_name`, `family_name`, `hd`); providers now return a `ProfileSnapshot` from their own vocabulary. GitHub answers with `avatar_url` and a single free-text `name`, so before this a GitHub login produced a user with no avatar and no name — silently, every field being optional.
- **Identity is the GitHub numeric user id, never the login.** A login is renameable by its owner and, once released, registrable by someone else, so keying identity on it would make a rename read as a new person and a recycled handle inherit a stranger's account and grants. allauth's `GitHubProvider.extract_uid` already returns `str(data["id"])`, so the durable subject reaches `ExternalIdentity.(provider_id, subject)` unaided. The login is never a gate either: **no clause in `evaluate_access` compares one.** An earlier revision of this requirement carried an `allowed_logins` clause and an `owner_only` that accepted an owner-login match, on the reasoning that an authorization filter re-checked on every login has a different standing from an identity key. That reasoning was wrong, and the AI review of `PR# 688 - tap` said so: re-checking a handle on every login still admits *whoever holds the handle at that moment*, so a released-and-re-registered login was a way in — and because `owner_only` combined its id and login tests with OR, a numeric-id **mismatch** was ignored whenever the login matched. Both clauses are removed. A login is display and log vocabulary; the numeric id is the only thing either identity or policy compares.
- **The access policy lives in `evaluate_access`**, the existing per-provider seam, not a second gate. Its clauses are a union; at least one must be configured:
  - `allowed_user_ids` — numeric ids. The only way to name an account in committed config.
  - `owner_only` — "only the account that created this instance". The intent is declared in the profile; the VALUE is derived at boot from the environment (`TAP_AUTH_INSTANCE_OWNER`), so committed config never hardcodes a handle. This is the shape the single-operator Codespaces standup needs. The owner resolves by **numeric `user_id` only**; an owner supplied as a `login` alone is unresolvable, FAILs the offline self-test with a message naming the missing id, and denies every login with `policy_unresolvable`.
- There is no "any GitHub account" login, for the same reason `google_oidc` requires `allowed_domains`: GitHub sign-up is open to the world, so an unset policy would publish the instance. An unset policy FAILs the offline self-test.
- A **declared but unresolvable** `owner_only` denies with its own reason code (`policy_unresolvable`) and FAILs the offline self-test. It never degrades to allow-anyone: a policy that exists and has no value to enforce is the presence-not-correctness shape, and the safe reading of "no owner known" is "nobody".
- **Verified email is re-expressed, not dropped.** `google_oidc` requires a provider-asserted verified email as the precondition for access because its gate *is* the domain of that email. GitHub's gate is not email-derived, so the requirement moves:
  - Access does **not** require a verified email — refusing an explicitly allowlisted numeric id over an unrelated missing address would deny for a reason the policy never expressed.
  - *Trusting* an email does require verification. GitHub's `/user` `email` is **self-asserted** (the account holder types it in), so it is never promoted to `AccessDecision.verified_email`. Only addresses GitHub itself marks verified are eligible.
  - This is load-bearing: `AccessDecision.verified_email` becomes `User.email`, and `User.email` is the key of the `TAP_AUTH_INITIAL_GRANTS` role map. A self-asserted address matching an initial-admin entry would otherwise be an unauthenticated grant of `tap_admin`. The social adapter's old `or claims.get("email")` fallback — unreachable under `google_oidc`, where an allowed decision always carries a verified email — was exactly that path, and is removed.
  - The verified set costs **no extra API call**: allauth already fetches `/user/emails` when `SOCIALACCOUNT_QUERY_EMAIL` is on and hands it over as `SocialLogin.email_addresses` (it strips it back out of `extra_data`). The social adapter normalizes that list into the claims under one namespaced key, so every provider sees verification the same way and nothing re-derives a fact allauth already holds.
- **What "live" means for an OAuth2 provider.** There is no discovery document, and "GET api.github.com answers" is a check that can only fail when GitHub is down — a green light for a misconfigured app, which is the false-green shape. What *can* fail is the credential pair, so the live self-test presents it at the code-exchange endpoint with a deliberately invalid authorization code, making GitHub adjudicate the credentials before it looks at the code:
  - `bad_verification_code` → PASS (the request got past credential validation, so the OAuth App is real and live).
  - `incorrect_client_credentials`, or HTTP 404 `Not Found` for an unknown client id → FAIL. Both answers verified against the live endpoint 2026-09-20.
  - anything else → WARN with the raw code echoed. An unrecognised answer is not evidence of health.

  The probe has no side effects: the code is syntactically impossible as a real grant, so nothing is consumed and no session is minted, and the client secret goes only to GitHub's own endpoint.
- **Exactly one `github_oauth` provider per instance.** allauth's OAuth2 URL builder emits a single `github/login/callback/` with no `provider_id` segment, so a second app is unreachable by construction — `get_app` raises `MultipleObjectsReturned` mid-login. The offline self-test refuses the configuration at boot, where it is legible, instead of at the callback, where it is not.
- The APPS entry still carries `provider_id`: allauth stamps `SocialAccount.provider` from `Provider.sub_id` (`app.provider_id or app.provider`), and that value is what the social adapter looks the TAP policy up by. Omitting it would stamp `"github"` and miss the policy.
- Deliberately **not** built, named so their absence is not mistaken for coverage:
  - `allowed_orgs` — org membership is absent from `/user`; enforcing it needs the `read:org` scope, a separate API call, and a decision about private membership. A declared-but-unenforced org policy would be a policy that exists and is false.
  - GitHub Enterprise Server — allauth's `GitHubOAuth2Adapter` reads `GITHUB_URL` at class-definition time, so an Enterprise base URL is process-global and not expressible per APPS entry.

##### Operator setup (what creating the OAuth App requires)

A GitHub OAuth App is a credential-bearing artifact and is created by a human, never by TAP. The complete list:

1. **App type: an OAuth App**, not a GitHub App. *Settings → Developer settings → OAuth Apps → New OAuth App*. (A GitHub App is an installable identity for acting on repositories; user sign-in needs an OAuth App, and allauth's `github` provider speaks that flow.)
2. **Authorization callback URL**: `<TAP_BASE_URL>/auth/github/login/callback/` — exactly, with the trailing slash. The path is fixed and carries no provider id, unlike the OIDC callbacks.
3. **Homepage URL**: `<TAP_BASE_URL>`. Not load-bearing.
4. **Scopes**: nothing to configure. GitHub OAuth Apps do not pre-declare scopes; the authorization request carries them, and allauth requests `user:email` (because `SOCIALACCOUNT_QUERY_EMAIL` is on) so GitHub's *verified* addresses are available.
5. **Credential**: generate a client secret and place it as the auth-scoped secret the provider resolves — `auth:<provider id>` by default, i.e. `<TAP_SECRETS_ROOT>/<provider id>.secret.json`, with `scope: "auth"`, `kind: "oauth2_client"`, and `data: {client_id, client_secret}` (`tap_auth/schemas/oauth_client_secret.schema.json`). Place it through the secret-provisioning path; it never enters the repository or a boot profile.
6. **Boot profile**: add the provider entry under `auth.providers` with `type: "github_oauth"` and at least one policy clause (prefer `allowed_user_ids`; a login's numeric id is `https://api.github.com/users/<login>` → `id`).
7. **Verify before logging in**: `manage.py auth_selftest --live` must report PASS for both `secret` and `client_credentials`. A FAIL there is a wrong credential, not a wrong login.
8. **Log in**: `/auth/login/` lists the provider as a POST button. `SOCIALACCOUNT_LOGIN_ON_GET=False`, so visiting `/auth/github/login/` directly does not start the flow — the form POST does.

#### Acceptance Criteria

| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-tap-auth-github-oauth-1 | Second Provider Type | Implemented | `github_oauth` is registered in the provider registry alongside `google_oidc` and declares allauth's `github` engine; `build_socialaccount_providers` groups entries by the engine each provider declares rather than a hardcoded `openid_connect`. | |
| req-tap-auth-github-oauth-2 | Numeric-Id Identity | Implemented | The durable subject is GitHub's numeric user id, never the login: a login rename produces the same `ExternalIdentity` row, not a second one. | |
| req-tap-auth-github-oauth-3 | Access Policy Required | Implemented | A `github_oauth` provider must configure at least one of `allowed_user_ids` / `owner_only`; an unset policy FAILs the offline self-test. There is no any-GitHub-account login, and no allowlist-by-login at all. | |
| req-tap-auth-github-oauth-4 | Outside-Policy Refused | Implemented | A GitHub account outside the configured policy is denied in `evaluate_access` with `account_not_allowlisted`, proven by a negative test rather than by a successful login. | |
| req-tap-auth-github-oauth-5 | Owner-Only Derived | Implemented | `owner_only` declares intent in the profile while the owner's value is derived from the environment, and matches the owner's numeric id only — never the owner's login, whose match would ignore an id mismatch. An unresolvable owner (including one known by login alone) denies with `policy_unresolvable` and FAILs the offline self-test, never degrading to allow-anyone. | |
| req-tap-auth-github-oauth-6 | Verified Email Only | Implemented | `AccessDecision.verified_email` is set only from an address the provider itself asserts as verified; GitHub's self-asserted `/user` email never becomes `User.email`, which keys the initial-grants role map. | |
| req-tap-auth-github-oauth-7 | Live Credential Check | Implemented | The live self-test adjudicates the client credentials at GitHub's code-exchange endpoint with a deliberately invalid code: `bad_verification_code` PASSes, rejected credentials FAIL, an unrecognised answer WARNs. It is never a SKIP reported as green. | The OAuth2 answer to `req-tap-auth-google-oidc-4`'s discovery check. |
| req-tap-auth-github-oauth-8 | Allauth Settings Shape | Implemented | `build_allauth_settings` emits exactly the keys allauth's app builder consumes (`provider_id`, `name`, `client_id`, `secret`, `settings`), with `provider_id` carried so `SocialAccount.provider` resolves back to the TAP provider's policy. | |
| req-tap-auth-github-oauth-9 | Single GitHub Provider | Implemented | A second configured `github_oauth` provider FAILs the offline self-test, because allauth routes every GitHub app at one callback path and would otherwise fail mid-login with `MultipleObjectsReturned`. | |
