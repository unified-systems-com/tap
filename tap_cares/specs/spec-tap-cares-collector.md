...
| req-tap-cares-collector-run-collection-16 | The Seal Refuses A Foreign Batch | Implemented | Before sealing, the batch's `source` must be `"tap_cares.collector"` and the job's own `batch_id` must be that batch. Either mismatch is refused and logged at ERROR. | The id arrives in a task payload next to a separately-supplied job id; without a derived binding a wrong argument seals somebody else's batch under the collector actor. |
