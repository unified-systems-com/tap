...
- **Distribution:** dist `zizmor-tap`; import namespace `tap_plugin.zizmor`; AppConfig `tap_plugin.zizmor.apps:ZizmorConfig`; standalone repo `unified-systems-com/tap-plugin-zizmor` from the first commit (every plugin is evicted; development runs through `spawn-session.sh <label> --boot-file <record> --dev-plugins zizmor,github_core`). A leaf plugin, not a `*_core` substrate: it consumes github_core's vocabulary and is consumed by git-serious.
