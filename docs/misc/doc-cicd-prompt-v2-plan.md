...
| 7 | Companion: agentic-actions-auditor nine-vector pass against `ai-review*.yml` + both harness workflow sets; findings (or clean bill) recorded here | **DONE 2026-08-31** — see §Companion below: 10 verdicts — 8 hold (one as "holds today"), 2 findings, both fixed/filed same day |
...
| Marker-comment adoption | **FINDING 2 (filed)** | Comment job adopts any comment starting with the marker, author unchecked — a hostile commenter can pre-seed a forged review, and the bot then updates a comment the attacker owns — and comment authors retain edit/delete rights, so the attacker can rewrite or remove the bot's review content at will afterward. Filed with fix shape: unified-ai-review#5 (includes the tap `pr-review-triage` SUSPECT-rendering half) |
