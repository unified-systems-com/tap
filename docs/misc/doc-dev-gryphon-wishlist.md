...
| Issue | Status | Detail |
| --- | :---: | --- |
| A variable reused INSIDE one pattern is not unified (wrong rows) | Open — found 2026-09-22, in repair | `MATCH (p)-[:A]->(x)-[:B]->(o)<-[:B]-(p) … RETURN p.name` returned **4 rows (local, local, foreign, foreign) where 1 (local) was correct**. The closing `(p)` appears to bind as a FRESH variable, so the result is the cross product of the two positions — the counts fit that exactly (INFERRED from counts by the finder, not yet traced in code). Suggested minimal repro: `(a)-[:X]->(b)<-[:Y]-(a)` on a triangle-free fixture. **The asymmetry is the tell:** reuse ACROSS clauses is already refused loudly (`Variable 'p' is bound in MATCH clause 1 and again in MATCH clause 2`, tap#433, `req-grid-traversal-lang-shape-5/7`) and a comma join is refused (`Advanced executor requires exactly one pattern per MATCH clause`) — only reuse WITHIN a single pattern is silent. Hunch: the single-MATCH advanced executor's pattern→ORM lowering names each node position independently and never adds the equality join. Found by `highbar` while fixing a cross-org leak on okta-tap#3. **Workarounds in COMMITTED code to remove once fixed:** okta-tap main `4d8fe50` uses an org-to-org two-variable shape across 13 scene edge searches + 5 tables (okta `specs/spec-okta-v0.md:223`, `specs/okta-corpus.md:147`), and mid-path nodes use a denormalised `v.data.org_name` filter. |
| A dotted path into `dimensions` silently returns 0 (wrong answer) | Open — found 2026-09-23, in repair | For a dimension key that itself contains a dot — the house convention, e.g. `tap.cloud`, `deployment.environment.<env>`, `git.host` — `WHERE n.dimensions.tap.cloud IS NOT NULL` returns **0 rows** while `WHERE n.dimensions["tap.cloud"] IS NOT NULL` returns the row. No error either way. Backtick quoting is not supported by the grammar (`No terminal matches '`'`). Hunch: the path lowers to nested JSON keys (`dimensions`→`tap`→`cloud`) and a missing JSON key under 3VL yields NULL/false rather than an error — so the fault is in path lowering or missing-key semantics, not the parser. An earlier probe with a dot-free key (`dcom`) passed, which is why it went unnoticed. Found by `highbar` on the live `highbar__dev` stack. **Open design choice:** reject a dotted path into `dimensions` with an error naming the bracketed form, or try the whole dotted string as one key first. Workaround today is documentation only (deployment-environment-tap#2 teaches the bracketed form and a test asserts it). |
...
| Dotted dimension key via dot-steps returned a silent zero rows (**B2**) — `Issue# 781 - tap` | Resolved 2026-09-23 (unreleased); design ruled by the owner | `MATCH (n:…) WHERE n.dimensions.tap.cloud IS NOT NULL RETURN n` returned **0 rows, no error**, against entities carrying `{"tap.cloud": "aws"}`; the bracketed spelling `n.dimensions["tap.cloud"]` returned the row. Cause (traced, not guessed): both JSON-spine resolvers (`_typescan_orm_path`, `_orm_path_for_envelope_path`) `__`-joined every step after `dimensions` individually, so `dimensions.tap.cloud` lowered to `entity__dimensions__tap__cloud` → `dimensions #> ARRAY['tap','cloud']` (a **nested** path), while the bracket form lowered to `dimensions -> 'tap.cloud'` (a **flat** key). `Entity.dimensions` is flat by construction with dotted key names (`req-grid-dimension-em`), so the nested reading can never match; under 3VL the missing path is NULL and `IS NOT NULL` / `=` were quietly false. This hit essentially **every real TAP dimension key** — the house convention is dotted (`tap.cloud`, `git.host`, `deployment.environment.<env>`) — and went unnoticed only because the first probe used a dot-free key (`dcom`). **Ruled 2026-09-23 by the owner (option a):** a path of more than one step after `dimensions` is REFUSED with an error naming the bracketed form, and is never reinterpreted as a whole key (`req-grid-dimension-query-form` in `spec-grid-dimension.md`, `Issue# 784 - tap`; the traversal-language half is `req-grid-traversal-lang-envelope-paths-9`). One step stays legal in either spelling. **The rival was built first and then reversed:** the initial fix treated a maximal run of dot-steps as ONE key, so `n.dimensions.tap.cloud` ≡ `n.dimensions["tap.cloud"]` and worked. That was declined — it makes one query text mean two things depending on the data, and a genuine typo would still answer with a silent zero. Counting STEPS rather than dots is what also refuses `["a"]["b"]`; admitting the bracketed nesting would only relocate the same bug. Both resolvers route through one helper `_json_spine_inner_path` so the refusal cannot apply at one site and not the other. Locked by `TestGryphonDimensionsMultiStep` in `tap_grid/tests/test_gryphon.py` (bracketed presence/equality/non-match/absent-key, the single-dot-step boundary, dotted refused naming the bracketed form, three-segment refused, bracket-nesting and mixed refused, never-reinterpreted, and BOTH dispatch sites refusing) — all eight refusal cases go red when the guard is reverted. Gridkin: `dimensions.gridkin.json` scenarios 1 and 4 are rejections. |
| Backtick-quoted identifiers unsupported by the grammar | Open — worth revisiting, and more interesting than it was | ``n.dimensions.`tap.cloud` `` is a parse error ("No terminal matches '`'"). Found alongside B2 on 2026-09-23. **Its weight changed with the 09-23 ruling:** the earlier note here said backticks were mere Cypher-parity ergonomics *because* the dotted and bracketed spellings both worked. They no longer both work — the dotted form is refused — so bracket is now the ONLY way to address a dotted house key, and backticks would be the second. Still not a correctness gap (the bracket form is complete and is what the error message names), so it stays a separate ergonomics decision. |
| A variable reused INSIDE one pattern was not unified (**B1**) — `Issue# 780 - tap` | Resolved 2026-09-23 (unreleased) | `MATCH (p)-[:A]->(x)-[:B]->(o)<-[:B]-(p) ... RETURN p.name` returned **4 rows where 1 was correct**, with no error — reported by session `highbar` 2026-09-22 against okta data, reproduced on `grid_fixtures` vocabulary. The neighbouring spellings were all already fenced: reuse across `MATCH` clauses raises (`req-grid-traversal-lang-shape-8`) and a comma join raises ("exactly one pattern per MATCH clause"); only reuse within one pattern was silent, because it is the only one the linear chain lowering can actually express. Cause (traced, not guessed — the reporter's cross-product reading was close but wrong in mechanism): `_build_var_bindings` keeps the FIRST occurrence of a name and skips the rest (`if ... not in bindings`), and `_build_chain_queryset` names each node position independently, so the repeat position was not a fresh binding — it was **no binding at all**, an unconstrained join position contributing only its own label filter. The captured SQL showed it: the second hop was joined and its endpoint Entity never was. Fix: `_repeated_variable_equality_filters` emits an identity equality from every repeat position back to the variable's first, folded into the same `.filter()` as the structural filters, so it compiles to `tap_edge.from_entity_id = T4.from_entity_id` — one SELECT, no extra join. Rejection was the rival fix and was declined: the join is expressible here, Gryphon has no var-to-var `WHERE` comparison to offer as a remedy, and apply beats refuse when apply is available. Two adjacent shapes ARE refused, because their executors cannot unify: one name used for both a node and an edge, and `OPTIONAL MATCH` reusing its mandatory variable. `req-grid-traversal-lang-shape-9`; locked by `TestGryphonInPatternVariableReuse` (`tap_grid/tests/test_gryphon.py`) and `TestInPatternVariableReuseIsAJoin` + four construct-effect cases (`tap_grid/tests/test_gryphon_construct_effect.py`). |
| A retired edge reached through a join was still matched (wrong rows) — `Issue# 811 - tap` | Resolved 2026-09-25 (unreleased; Part 1 of the read scope) | `MATCH (a)-[:L]->(b)-[:L]->(c) WHERE a.entity_id = $a RETURN c.name` returned **1 row where 0 was correct** after the `b->c` edge was retired, while retiring the `a->b` edge returned 0. Also measured on `grid_fixtures` before the fix: the same defect written right to left (1 row, expected 0); a live edge onto a tombstoned endpoint (rows `B, D`, expected `B`; envelope nodes `A, B, D`, expected `A, B`); an OPTIONAL MATCH out-degree counting a retired edge (2, expected 1); a NOT EXISTS guard chain broken at its second hop still guarding its target. Cause (traced): liveness was a side effect of `LiveManager`, which filters only a queryset's ROOT model; every relation reached through `edges_out` / `edges_in`, both endpoints, the NOT EXISTS inner pattern and the OPTIONAL MATCH `Count(filter=...)` were unscoped. Fix: `req-grid-traversal-exec-read-scope` — one scope module (`tap_grid/gryphon/read_scope.py`) every read is built from, and a compiled-query check that refuses to execute a read with any unscoped spine or grid-table alias. Locked by `tap_grid/tests/test_gryphon_read_scope.py`. The Gridkin property campaign across all shapes is the ruled follow-up (Part 2). |
| NOT EXISTS correlated a far inner node through a second, untyped join (wrong rows) | Resolved 2026-09-25 (unreleased) | Found while building the read scope, independent of liveness. `NOT EXISTS { MATCH (g)-[:GUARD]->(h)-[:GUARD]->(t) }` applied its `OuterRef` correlation on `t` in a separate `filter()`; `t` sits past the inner root edge, on a multi-valued relation, so Django added a second join of ANY edge from `h` onto `t`. An `h -[:OTHER]-> t` edge then satisfied the correlation while `h`'s `GUARD` edge to somewhere else satisfied the structure, and `t` was wrongly treated as guarded (measured: `['D']` where `['B', 'B', 'D']` was correct). The inner WHERE had the same shape. Fix: the correlation and the inner WHERE fold into the inner chain's one `filter()` (`req-grid-traversal-exec-read-scope-7`); locked by `test_not_exists_correlates_on_the_structural_hop`. |
...
- [`plan/road-products.md`](../../plan/road-products.md) — The products roadmap: the demand/intent layer, carrying each step's Objective, Done-Test and Non-Goals. Active step is `step-products-git-serious-self`. No dates live here (`req-roadmap-dates-in-tracker`) — a step carries a `Tracking:` pointer and the milestone behind it owns the date. The Doctrine section is the standing strategic filter against which wishlist promotions get evaluated.
- [`specs/spec-roadmap.md`](../../specs/spec-roadmap.md) — Roadmap conventions (step IDs, status vocabulary, and the tracker pointer that replaced the in-doc timeline table).
