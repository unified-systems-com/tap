...
| Dotted dimension key via dot-steps returned a silent zero rows (**B2**) — `Issue# 781 - tap` | Resolved 2026-09-23 (unreleased); the error-vs-whole-key-first design choice is NOT yet ruled | `MATCH (n:…) WHERE n.dimensions.tap.cloud IS NOT NULL RETURN n` returned **0 rows, no error**, against entities carrying `{"tap.cloud": "aws"}`; the bracketed spelling `n.dimensions["tap.cloud"]` returned the row. Cause (traced, not guessed): both JSON-spine resolvers (`_typescan_orm_path`, `_orm_path_for_envelope_path`) `__`-joined every step after `dimensions` individually, so `dimensions.tap.cloud` lowered to `entity__dimensions__tap__cloud` → `dimensions #> ARRAY['tap','cloud']` (a **nested** path), while the bracket form lowered to `dimensions -> 'tap.cloud'` (a **flat** key). `Entity.dimensions` is flat by construction with dotted key names (`req-grid-dimension-em`), so the nested reading can never match; under 3VL the missing path is NULL and `IS NOT NULL` / `=` were quietly false. This hit essentially **every real TAP dimension key** — the house convention is dotted (`tap.cloud`, `git.host`, `deployment.environment.<env>`) — and went unnoticed only because the first probe used a dot-free key (`dcom`). Fix: one shared helper `_json_spine_inner_path` behind both resolvers — a maximal run of dot-steps is ONE key (names rejoined with `.`), a bracket-key step is one key and ends the run (`req-grid-traversal-lang-envelope-paths-9`). Rejecting the dotted spelling was the rival fix and was declined: it would cost the natural spelling for every real key, and there is no second valid reading to disambiguate from. Locked by `TestGryphonDimensionsMultiStep` in `tap_grid/tests/test_gryphon.py` (flat dotted key, 3-segment key, dot-free-key regression pin, non-matching value, absent key, byte-identical-SQL equivalence at BOTH dispatch sites, and the bracket nested-walk escape hatch). |
| Backtick-quoted identifiers unsupported by the grammar | Open — not scoped | ``n.dimensions.`tap.cloud` `` is a parse error ("No terminal matches '`'"). Found alongside B2 on 2026-09-23 and deliberately **not** fixed with it: with `-9` in place the dotted and bracketed spellings both work, so backticks are an ergonomics question (Cypher parity), not a correctness one. Decide separately. |
