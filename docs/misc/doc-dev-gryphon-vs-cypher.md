...
| Multiple `MATCH` clauses | Second `MATCH` is evaluated **per row** of the first and **joins**; earlier bindings stay in scope | Each clause executes **independently**; results are **unioned and deduplicated** by `entity_id`. Earlier bindings are **not** in scope — a reused variable name is a fresh binding, not a join. Expecting the Cypher meaning returns a deduplicated superset **with no error**. Whether union is the intended semantics is open: tap#433 | `req-grid-traversal-lang-shape-5` (*Multiple MATCH — Union, Not Composition*); the unbuilt join is `-shape-7` |
