...
| Multiple `MATCH` clauses | Second `MATCH` is evaluated **per row** of the first and **joins**; earlier bindings stay in scope | Each clause executes **independently**; results are **unioned and deduplicated** by `entity_id`. Earlier bindings are **not** in scope — a reused variable name is a fresh binding, not a join. A variable reused across clauses — the Cypher join, spelled out — is **rejected** with the remedy (one `MATCH` for one path; distinct names for independent scans), so the superset can no longer come back silently. Ruled tap#433 (2026-09-11): union is the intended semantics; composition is a known gap | `req-grid-traversal-lang-shape-5` (*Multiple MATCH — Union, Not Composition*); the unbuilt join is `-shape-7` |
