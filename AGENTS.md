...
8. **Write programmatic layouts through the layout skill.** When a natural layout plus tags cannot express
   an arrangement, a JavaScript layout is welcome if it follows the
   [`create-layout`](tap_viz/skills/create-layout/SKILL.md) skill, which configures the tap_viz runtime
   rather than re-implementing it, so layout code stays generic and shared.
...
- **Direct ORM writes — the code rule and the agent rule are different, and the agent rule is a hard stop.**
  ...
  - *As an agent in a session, you do not write one.* Not in a shell, not in a script, not temporarily, not "just to check something" — and **you do not add a `TAP-WRITE-COV` annotation to make one pass**, which is the obvious loophole in the rule above. If you believe a direct ORM write is required, **halt and raise it with the maintainer before writing anything**. The interruption is the point: a case that looks like an exception is usually a missing service-layer verb, and writing the bypass hides the gap instead of surfacing it. Closing `unified-systems-com/tap#730`, which is what an undefined standing exemption ("out-of-scope admin/infrastructure", claimable by assertion) cost.
