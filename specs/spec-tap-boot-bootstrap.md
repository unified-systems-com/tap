...
| req-boot-bootstrap-samsite-rehome-5 | CI Lane Pointer-Fetches | Retired | The samsite CI lane fetches the record via pointer — no lane-local copy; since tap#364 the pointer's rev derives from `boot/test_all.boot.json`'s samsite pin (it had drifted to `@v0.2.0` against `v0.2.4`). | **Retired 2026-09-25 (tap#638):** the core-side `samsite` lane was removed from `product-lines.yml`, so there is no lane left to fetch anything. The record is exercised by tap-plugin-samsite's own CI (`-rehome-4`) and nightly against core `main` by `nightly-plugins.yml`. The product-journey successor (tap-plugin-samsite#4) had not landed; the removal went ahead on George's approval, and `spec-dev-validation.md` § Per-Product-Line CI Lanes names the gap. |
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-boot-bootstrap-ci-record-1 | Reserved Name | Implemented | `ci` is a reserved record name; a plugin's test stack ships at `tap_plugin/<slug>/boot/ci.boot.json`, declared in `[[boot.records]]` with its `sha256` like any other record. | Second reserved name after `default` (`req-boot-bootstrap-default-record`). `validate_plugin`'s `ci-record` check fails a record that exists but is undeclared, or declared with a digest that does not match its content (`tap_plugins/tests/test_validate_ci_record.py::TestPresence`). |
| req-boot-bootstrap-ci-record-2 | In-Package, Not Repo-Root | In Development | The CI stack is wheel-borne and hash-covered. A repo-root `ci/nightly.boot.json` is the retired form: unshipped, unhashed, unfetchable, undiscoverable. | Migration is per-plugin and mechanical — tap#365's per-repo sub-issues. The retired form survives only as the deprecated `boot_profile` input → `validate_plugin --ci-record`, reported as a deprecation notice. |
| req-boot-bootstrap-ci-record-3 | Closed Over Declared Deps | In Development | The install set is exactly `depends_on` closure + self; an undeclared plugin fails `validate_plugin --strict`. | The rule that makes per-plugin CI authoritative. Author-time: every declared dependency and self must be installed (fail), entries beyond that are reported as assumed-transitive (`_check_ci_record_content`); the closure itself is only verifiable at boot by the pre-boot dependency gate. |
...
| req-boot-bootstrap-ci-record-5 | Offline + Credential-Free | Implemented | No source `credential`, no `required_secrets`, no network-reaching population step — so a fork can run it and a nightly cannot flake on a third party. | `validate_plugin` fails a `ci` record with `required_secrets` or a source `credential` (`test_credentials_and_secrets_fail`); a network-reaching population step is not yet detectable at author time. A plugin genuinely needing a credentialed dep declares that cost explicitly rather than smuggling it in. |
| req-boot-bootstrap-ci-record-6 | Locked Across Plugins | In Development | Every plugin repo ships a `ci` record; the lane's depth becomes default-on, and opting out is a declared exception rather than a missing file. | The inversion landed: `validate_plugin --strict` (the conformance gate) FAILS without the record (`test_absent_warns_and_strict_fails`), so opting out is now impossible by omission. Completes when every plugin repo ships the record (tap#365 sub-issues; 1 of 16 does on 2026-09-09: gryphon_playground). |
