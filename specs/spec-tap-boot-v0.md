...
| req-boot-preboot-3 | Reboot Is A No-Op | Implemented | `is_satisfied()` skips an already-installed plugin (git: installed PEP 610 `commit_id` equals the pinned `commit` — a tag-only entry never matched and reinstalled every boot until it pins one, `req-boot-bootstrap-install-commit-pin`; editable/path: present). Validated: a re-run logs "already satisfied — no-op". | Offline `wheelhouse` idempotency (dist present at the wheel version) lands with `req-tap-plugin-arch-sources-6` |
...
> **v0 phase order: `posture → auth → grid_infra → population → web`** (see [v0 Scope](#v0-scope-minimal-useful)).
>
> Updated 2026-09-16. The order had been recorded as `auth → population` while the code had already grown `grid_infra` and `web`, and `posture` was added ahead of `auth` by tap#272 — the deploy security posture gate runs before any write, and its answer does not depend on the profile. Each addition is what this requirement's last bullet describes: a new phase added in code with explicit placement, never declared by a profile. Recorded here because a stated phase order that no longer matches the code is a declaration that reads as verified and is not. The fuller `bootstrap → identity → auth → population` described below is the future shape: v0 needs neither a separate `bootstrap` actor-resolve phase nor an `identity` keystone phase, because nothing writes through the service layer before `auth`. In v0 the boot actor is resolved *in* the `auth` phase (after `sync_auth` creates it) and bound (`acting_as`) for `population`.

- Coarse phase order is hardcoded, not config. **v0: `posture → auth → grid_infra → population → web`** (the fuller order below is added in code as those phases gain content).
