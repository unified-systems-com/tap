...
#### `detail` is authored, never a raw exception message (tap#546)

A projection can only withhold what a probe *put somewhere*; it cannot make a trusted projection safe to persist. `req-tap-health-exposure-6` made that distinction load-bearing: the image `HEALTHCHECK` runs the tier-1 CLI every 120s, and Docker concatenates each run's stdout into the container's `State.Health.Log`, which `docker inspect` exposes and container-monitoring agents routinely scrape. The CLI is a *trusted* surface and must keep printing `full()` — the spawn gate and every human debugging a boot depend on it — so narrowing the CLI's projection is **not** the remedy, and neither is filtering the string on the way out (a redactor is drift-detection wearing prevention's clothes; it ships the lie first).

The remedy is upstream, at the one place the fact is derived (remedy 1, *derive a fact once*): a probe's `detail` is an **authored** short string. A caught exception contributes its **TYPE name only**, via the single helper `tap_health.results.exception_detail()` — never `str(exc)` and never an f-string interpolating it. A `psycopg` `OperationalError` routinely carries host, port and role, and nothing proves a DSN or credential material cannot reach that message on some path; Docker's per-entry truncation is not redaction, so a partial leak is still a leak. Bounding the probe fixes every projection *of the probe result* at once — the `Health.Log` entry Docker takes from the CLI's stdout, the CI log the spawn gate writes, and any future authorized API — instead of one of them. It does **not** by itself make `State.Health.Log` free of exception text; see the known gap below.

The check stays **useful** because usefulness never rested on the exception text: the probe `name` says which probe failed and the stable `code` says how (Law 4), and both survive. This applies to the runner's isolation path (`probe.raised`) as well as to probes, and to every app that contributes a probe — `tap_grid`, `tap_auth`, `tap_plugins`, `tap_cares`, `tap_web` — not only to `tap_health`'s own. It is **enforced rather than agreed**: `tap_health/tests/test_probe_detail_leak.py` fails a probe result built with a canary-bearing exception, and an AST guard in the same file constrains the **source** — inside an `except … as exc` handler of any first-party module that builds probe results, referencing `exc` at all is a finding unless it sits in one of three sanctioned contexts (`exception_detail(exc)`, a `logger.<level>(…)` call, or a `raise … from exc`). Constraining the source rather than matching the `ProbeResult` call is what closes the launder-through-a-local (`detail = str(exc)`) and module-alias (`PR.unhealthy(…)`) bypasses: the message is never derived in the first place. The modules are discovered by scan, not listed, so a probe module added later is covered the day it lands.

Known gap, deliberately not closed here: probe modules also `logger.warning("…: %s", exc)`, and the logging console handler writes to **stderr**, which Docker also captures into `State.Health.Log`. That is a separate remedy (an invocation/logging decision, not a probe-detail one) tracked as tap#681; closing tap#546 does not by itself make `Health.Log` free of exception messages.
