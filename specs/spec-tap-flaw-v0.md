...
- **Any instance running where the developer is not.** The demand was first written under `step-rampart-first-paying-customer` (now Superseded) and is unchanged by the step closing: operating a field instance safely requires a clear "should-never-happen" signal so the developer is not paged for routine events and *is* paged — with enough context to patch — for genuine defects. The friends preview under `step-products-git-serious-self` is the next instance to meet this condition.
