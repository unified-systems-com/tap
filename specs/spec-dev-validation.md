...
| req-dev-validation-baseline-vocabulary | [Baseline Fixture Vocabulary](#baseline-fixture-vocabulary) | Implemented | Core tests build fixtures from a declared set of plugin-supplied node/edge vocabularies; a stack lacking them cannot exercise the grid spine and fails loudly rather than erroring file-by-file or skipping green |
...
| req-dev-validation-product-line-lanes | [Per-Product-Line CI Lanes](#per-product-line-ci-lanes) | Implemented | Validate each product line (a plugin-pack + boot profile) on its own free GitHub-hosted runner, in parallel across lines — parallelism along the product axis, not arbitrary shards. The `core_ci` line is the one line in tap's PR checks and the promote gate; the `test_all` PR line was eliminated (tap#369) and the `samsite` product line left with tap#638 — product lines are proven in their own repositories' CI. Ran on AWS CodeBuild 2026-07-08 → 2026-08-10, retired by the measured free-runner consolidation (identical job ~9 min; in-account-IAM rationale lapsed with the public plugin repos). |
| req-dev-validation-bom-lane | [The BOM Lane](#the-bom-lane) | Implemented | The bill of materials is the DECLARED input set (`tap/bom_inputs.py`: the boot records — the plugin-rev column — plus `uv.lock`, `pyproject.toml`, the image, compose, `.env`, the install code, a record's editable paths); the full pinned set boots in the real image and passes core + every installed plugin's shipped tests — nightly, when any BOM input changes (required through `gate`), and on the release candidate before the version tag is promoted (`bom-boot.yml`, tap#366) |
...
**A validation surface is not only a static check.** `req-dev-validation-map-1` says *every*
surface has a row, and that includes the **boot preflights** — the fail-closed checks a boot
runs on itself before it mutates anything (declared population secrets, collector liveness,
declared install credentials, the boot record's own presence). They enforce a class of failure
out of the system exactly as a guard or a gate does; the difference is only cadence (per-boot /
per-spawn rather than per-commit), which the row states. They were absent until 2026-08-27 —
a Map showing only guards and CI lanes implied that class was unguarded, and the
install-credential preflight got built precisely because that half had no enforcement at all.
The rule going forward: if it fails closed and keeps a defect class out, it earns a row,
whatever stage it runs in.
...
| architecture.md fitness | `req-docs-architecture-fitness` | Per-commit (`pytest`) | CI-guarded | `tap.guards.architecture_doc` (via `tap/tests/test_guards.py`) |
| Assembled-instance health | `req-tap-health-exposure` | Per-commit (`pytest`) + per-spawn (`manage.py health --set readiness` gate) + per-120s in-container (the image's `HEALTHCHECK`, req-tap-health-exposure-6) | Partially guarded — CI-guarded units + per-spawn exec gate; full live cold-boot run Named, deferred. The container `HEALTHCHECK`'s DECLARATION is CI-guarded; its runtime behaviour (reaching `healthy` without flapping, flipping `unhealthy` on a stopped database) is NOT OBSERVED — it needs a built image and a live stack | `tap_health/tests/` + `spawn-session.sh` health gate + `tap/tests/test_container_healthcheck.py` (the `HEALTHCHECK` instruction's argv, parsed by the health command's own parser); folds into the cold-boot cycle |
...
| Attestation preconditions (push-to-registry has credentials in its job) | `req-cicd-sbom-4` | Per-commit (`pytest`) | CI-guarded | `tap/tests/test_attest_preconditions.py` — static over `.github/workflows/*.yml`, because `publish-images` runs only on push to main, so PR CI cannot exercise the attest steps (tap#543: the signing split left the registry login in the job it split from and shipped green). |
| Authz coverage | `req-tap-auth-policy-9` | Per-commit (`pytest`) | CI-guarded | `tap.guards.authz` (via `tap/tests/test_guards.py`) |
| Baseline fixture vocabulary present for core runs | `req-dev-validation-baseline-vocabulary` | Per-commit (`pytest`, collection time) | CI-guarded | `tap.pytest_harness.pytest_collection_modifyitems` (fail-closed `UsageError` when a core run lacks `tap.plugin_testing.BASELINE_PLUGIN_SLUGS`); unit-guarded by `tap/tests/test_baseline_vocabulary.py` |
| BOM inputs declared once (`tap/bom_inputs.py`): change-tier and the uv-cache keys derive from it | `req-dev-validation-product-line-lanes-9` | CI (every PR: `scripts/change-tier` classifies the diff through `tap.bom_inputs --classify`) + pytest guard | Guarded — `BOM_INPUTS` names every input that moves the booted artifact (records, uv.lock, pyproject, the image, compose, .env, the install code, a record's editable path); `tap/tests/test_bom_inputs.py` proves each declared input classifies `boot` (drop-one), that the boot's read set is declared, and that every workflow uv-cache `hashFiles(...)` equals the declared `RESOLUTION_INPUTS` (tap#379) | `tap/bom_inputs.py`; `scripts/change-tier`; `tap/tests/test_bom_inputs.py`; `tap/tests/test_change_tier.py` |
| BOM lane: the full pinned set boots and passes its whole suite | `req-dev-validation-bom-lane` | Nightly (`bom-boot.yml` schedule) + CI when a boot record changes (`boot` tier, REQUIRED via `gate`) + dispatch; no longer on release tags (tap#638 — a release gates on core_ci) | Gate-guarded on the boot tier; nightly is signal. Boots `test_all` (the BOM) in the real image, runs `scripts/gate`, then `tap.lane_run`: the core walk and every installed plugin's shipped suite as separate invocations (pytest prunes a named plugin dir once the root is also an argument — the single-invocation shape ran core alone), membership derived from the record, per-owner collected/executed in the summary, the Gryphon corpus required to execute (tap#369) | `.github/workflows/bom-boot.yml`; `scripts/change-tier` (`boot`); `product-lines.yml` `gate` |
| Boot preflight — collector credential liveness (live self-test) | `req-boot-obs-preflight` | Per-boot (`manage.py boot`, after the offline lane, before population mutates) | Gate-guarded (fail-closed) — batch verdict; splits absent-secret (provisioning) from dead-credential (liveness), the distinction the self-test alone conflates | `tap_boot/orchestrator.py` preflight batch (collector self-tests); failing checks recorded per collector in the boot record |
| Boot preflight — declared install credentials (offline, enumerate-first) | `req-tap-plugin-arch-source-secret` | Per-boot (pre-boot, head of the install stage) + per-spawn (host-side, `spawn-session.sh` Step 2.9b and the `--dev-plugins` derivation) | Gate-guarded (fail-closed) — every declared credential checked in ONE verdict before the first install or clone; checks BOTH resolver rules (filename for host tools, `scope`/`key` identity for the container) so a store that satisfies one and not the other fails host-side instead of aborting pre-boot minutes later | `tap/install_credentials.py` (the one derivation, stdlib-only/host-runnable), called by `tap/preboot.py::_install_plugins`, `tap/dev_workspace.py`, and `spawn-session.sh` (exit 3); `tap/tests/test_install_credentials.py` + the ordering tests in `test_preboot.py`/`test_dev_workspace.py` |
| Boot preflight — declared population secrets (offline) | `req-boot-required-secrets` | Per-boot (`manage.py boot`, before any population step) + author-time (offline-checkable from profile + envelopes) | Gate-guarded (fail-closed) — presence + kind for every entry an enabled step consumes; abort names `scope:key` + expected kind, never a value | `tap_boot/orchestrator.py::_preflight_required_secrets` (verdict persisted to the boot record's `abort.missing_secrets`); profile-level coherence rules in `tap_boot/profile.py`; `tap_boot/tests/` + the `/provision-secrets` skill as the named operator consumer |
| Boot-profile preflight (record present before any container work) | `req-boot-bootstrap-samsite-rehome` | Per-spawn (host-side, `spawn-session.sh` Step 2.9) + per-boot (container-side readers) | Gate-guarded (fail-closed) — a rehomed/absent profile fails in <1s naming the available ids and the pointer form, instead of a crash-looping entrypoint after the image pull | `tap/boot_naming.py::profile_not_found_message` (the one Python home, raised by both container-side readers) + its shell twin in `spawn-session.sh` Step 2.9 |
...
| Copied-image SBOM facts derived from the Dockerfile pin | `req-cicd-sbom-3` | Per-commit (`pytest`) + per-publish (publish-images `sbom` job, before `attest-sbom`) | CI-guarded | `scripts/sbom/generate.py` `derive_copied_image_facts` — a copied-image component's version, source ref and purl are joined from the `COPY --from` pin that lands its path, and the JSON Schema plus the generator both refuse an entry that authors one of them (`tap/tests/test_sbom_generate.py`). Sits BESIDE the COPY --from reconciliation row, which checks paths only: for four uv releases that gate was cited as keeping the SBOM honest about versions, which it never was (tap#225) |
| Core PR lane: core + the fixture plugins (`core_ci`) | `req-dev-validation-product-line-lanes-8` | CI (every PR, `product-lines.yml` `line` → `core-ci.yml` — THE PR gate since the `test_all` line was eliminated, tap#369/#374; REQUIRED via `gate`) | Gate-guarded — boots `boot/core_ci.boot.json` (grid_fixtures, gryphon_playground, validation_sample — fixtures only since tap#638) and runs the core suite + the plugin contract suite, with the Gryphon corpus `--require`d to execute; bounded by `tap/tests/test_core_ci_profile.py` (exactly the fixture plugins; a non-empty population seeding a fixture that declares GRIFT). Runs through `tap.lane_run`: the core walk and each installed plugin's suite as separate owners, membership from the boot record, per-owner execution in the job summary (tap#369) | `.github/workflows/core-ci.yml` `line`, called by `product-lines.yml` `line`; `tap/tests/test_core_ci_profile.py` |
...
| DCO sign-off trailers (both roads to main) | `req-cicd-dco-signoff` | Pre-push (`scripts/check-dco`, wired into `promote-to-main.sh`) + CI (`product-lines.yml` `dco` job) | Gate-guarded (enforcing since 2026-08-12, when CONTRIBUTING.md + DCO landed at the repo root as approved policy; server-required through the `gate` aggregator only since 2026-09-18 — before that the `dco` job went red and blocked nothing, tap#353) | `scripts/check-dco` (every non-merge commit added over origin/main carries `Signed-off-by`; the ONE exemption is a pull request whose AUTHENTICATED author is an approved bot identity — `scripts/pr_bot_identity.py`, never the commit's author string, which the committer sets, tap#335); the trailer itself is applied by `.githooks/prepare-commit-msg` |
| Declared out-of-band components scanned for vulnerabilities | `req-cicd-security-scanning-5` | Nightly (`grype-nightly.yml`, 09:30 UTC; its own lane since tap#296) | CI-guarded (report-only) | `scripts/sbom/declared_cdx.py` emits an identity-only CycloneDX per supplemental; Grype scans it, `scripts/sbom/sarif_locate.py` stamps every result with the declaring manifest line (an SBOM scan has no file, and Code Scanning rejects a location-less SARIF — tap#294; `tap/tests/test_sbom_sarif_locate.py`), and the SARIF lands in code scanning under `grype-declared-<image>`. Grype and not Trivy because Trivy matches on purl-in-a-known-ecosystem and ignores `cpe`, which is the only identifier a self-compiled tarball can carry (measured: 0 findings vs 46). Honest status: report-only and deliberately unwaived — a finding needs the provider triage (`req-fips-pin-currency-7`) before it means anything, and nothing yet routes a finding to a human who is not looking at the Security tab (tap#231) |
...
| FIPS provider source integrity (digest + signature, both images) | `req-cicd-supply-chain-provenance-3` | Per-image-build (`publish-images.yml` on main; local `scripts/dc build`) | CI-guarded (fail-closed at publish) | `docker/build-openssl-fips.sh`, run by the `ossl-builder` stage of BOTH shipped Dockerfiles: sha256 against the repo pin, then the detached PGP signature asserted to name the authorized PRIMARY fingerprint; the signature check runs unprivileged under `--no-new-privs` on read-only inputs and self-tests its own privilege drop. Honest scope: images build on main push, not on PRs, so a bad pin merges green and fails the PUBLISH — nothing ships, but the red arrives after the merge. #231 is the missing upstream-drift watch |
| FIPS validation claims derive from the pin | `req-fips-pin-currency-8` | Per-commit (`pytest`) | CI-guarded | `tap.guards.fips_claims` (via `tap/tests/test_guards.py`) |
| Git plugin pin integrity (author-time: tag names the pinned commit) | `req-boot-bootstrap-install-commit-pin` | Per-PR (product-lines `pins` job, clean checkout) + per-commit (`pytest`, the checker itself) | Gate-guarded (fail-closed; forge-not-observable fails too) — tap's own `boot/` only; plugin in-package records are covered at AUTHOR time by the row below (493-D, tap#514), which checks the pair is present and well-formed without asking the forge | `python3 -m tap.git_pin --check boot/*.boot.json` in `.github/workflows/product-lines.yml` `pins` (in `gate`'s needs); `tap/tests/test_git_pin.py` (checker logic; deliberately NOT a glob of `boot/` — the test lanes stage released plugin records there, which gain `commit` per tap#493 sub-issue C) |
| Git plugin pin presence in a plugin's own `ci` record (author-time) | `req-boot-bootstrap-install-commit-pin` | Per-plugin (`validate_plugin`, incl. `--strict` in the reusable plugin-repo conformance job) | Conformance-guarded (fail-closed) — closes the documented gap where a plugin's in-package record could pin a MUTABLE tag with no commit and pass every gate; pre-boot already reported this but `observe_continue` reports after the wrong code installed, which is detect-tier. Deliberately asks NO forge, so it runs offline and in a fork PR: it checks the pair is PRESENT and a 40-hex sha, while `tap.git_pin --check` remains the one surface that verifies the tag still NAMES that commit | `tap_plugins.validate.service._check_ci_record_content` (`ci-record` check) → `tap.git_pin.is_commit_sha` — the one sha validator, shared with `tap.plugin_release`; `tap_plugins/tests/test_validate_ci_record.py` covers missing, malformed, and a positive control that a non-git source is never asked for a commit |
| Git plugin tag drift at boot (moved/missing tag, unpinned entry) | `req-boot-bootstrap-install-commit-pin` | Per-boot (pre-boot install) + per-commit (`pytest`) | Boot-reported (observe-continue security Flaw; never blocks — the pinned commit is what installs) — unit-guarded | `tap/preboot.py` `_check_git_pin` → `tap.git_pin.check_pin`; `tap/tests/test_preboot_commit_pin.py` |
| GRIFT import known-answer corpus (the batch playground) | `req-grid-batch-corpus-runner` | Per-commit (`pytest`, `-m batch_corpus`; in the fast lane) | CI-guarded | `tap_grid/batch_corpus/` scenarios run by `tap_grid/tests/test_batch_corpus.py` through the public `grift_import` surface — three truths compared: persisted state (rows, versions, spine, dimensions, typed fields, edge endpoints), the API report (result shape, counts) and provenance (event deltas, batch attribution), symbolic ids, the tombstone invariant after every scenario; the checker proven against corrupted runs on every layer (`test_batch_corpus_checker.py`), the oracle against the registry and a wrong hand answer (`test_batch_corpus_oracle.py`), metamorphic invariance (`test_batch_corpus_metamorphic.py`), two-writer schedules with both invariants (`test_batch_corpus_concurrency.py`) |
...
| Issue-link trailers (both roads to main) | `req-cicd-issue-link` | Pre-push (`scripts/check-issue-link`, wired into `promote-to-main.sh`) + CI (`product-lines.yml` `dco` job) | Gate-guarded (enforcing from 2026-09-03, tap#327; server-required through the `gate` aggregator only since 2026-09-18 — before that the `dco` job went red and blocked nothing, tap#353) | `scripts/check-issue-link` (the range over origin/main carries a qualified `Closes:` / `Part-of:` / `No-issue:` trailer); `scripts/promote-pr-body` derives the PR body's `## Issues` lines from the same parser, so GitHub auto-links and auto-closes; the CI bot exemption is by authenticated PR identity from `tap/tap.pr-bots.json`, derived once in `scripts/pr_bot_identity.py` and shared with `scripts/check-dco` (`req-cicd-issue-link-6`) |
...
| Live-API property fuzz — nightly exploration (random seed) | `req-dev-validation-api-fuzz` | Nightly (`api-fuzz-nightly.yml`, cron `47 9 * * *` + `workflow_dispatch`) | Loud, off the promote path (since 2026-09-11; report-only before) — the same reusable `api-fuzz.yml` in exploration posture: random seed, deep example budget, `fail_on_findings: true` reds the run, which no gate consumes. The red is READ on the double-tap landing page, which renders every collected `github_core__github_actions_run` (event, conclusion, age) — a red nightly and a nightly that never ran are both visible there; nothing files an issue (operator ruling 2026-09-14, tap#439) | `.github/workflows/api-fuzz-nightly.yml` → `api-fuzz.yml` (seed empty/random, `max_examples: 200`) — discovers NEW bugs off the promote path so a fresh finding never blocks a merge; the triage steps live in the workflow file beside the flag: artifact → reproduce by seed → fix → bump the gate seed |
...
| OpenSSL FIPS pin currency (upstream re-assertion) | `req-fips-pin-currency` | On demand (`scripts/verify-openssl-release`); scheduled run is OPEN | Manual (CI-unguarded by design, pending tap#231) | `scripts/verify-openssl-release`: re-checks the published sha256, the detached signature's presence, the signer's authorization AT THE RELEASE TAG, and that the committed key still exports the pinned primary — reading the pins FROM docker/build-openssl-fips.sh rather than restating them. Three-valued with distinct exit codes (0 holds / 1 changed / 2 NOT OBSERVABLE). Honest status: nothing runs it on a schedule, so the pins are only as current as the last manual run; CMVP certificate status is not observable at all. The bump path is the `bump-openssl-fips` skill |
...
| Per-plugin crypto posture (conformance) | `req-fips-crypto-bom-conformance` | Per-plugin (`validate_plugin`; `--strict` in conformance CI) | Conformance-guarded (warn; strict→fail) | `tap_plugins.validate` `crypto-providers` check → `tap.crypto_bom.scan_plugin` (plugin tree + the installed files of its declared distributions) → `tap.crypto_bom.ci_record_verdict`: ONE verdict — the boot gate's classifier and waiver matching applied to the plugin's own `ci` record, so an unwaived non-validated provider (found, or declared with no observable artifact) warns here and fails `--strict` exactly where boot would TAP-ABORT; unobservable dependencies are named, never clean; the `fips-posture:` line feeds the CI summary (tap#377) |
| Per-plugin repo CI (reusable workflow) | `req-tap-plugin-extdev-repo-ci` | Per-push/PR in each plugin repo (thin caller → `workflow_call`) | CI-guarded, fail-closed since tap#365 (2026-09-09) — CONFORMANCE runs for all 16 plugin repos (structure + requires_tap floor, Django-free, ~15s) and REFUSES a plugin that ships no in-package `ci` boot record, so boot-and-test (the plugin's OWN shipped tests in a real stack, against core at the plugin's declared floor) can no longer be skipped by omission. Roll-out state: 1 of 16 repos ships the record (gryphon_playground); the rest are tap#365's per-repo PRs, red until they land. Measured 2026-08-27: gryphon_playground ships 248 tests and 5 had rotted against a deliberate core change with nothing to catch it | `.github/workflows/plugin-ci.yml` — `validate_plugin --strict` (its `ci-record` check fails closed) against core checked out at the SHA `tap.ci_harness` resolves from the plugin's requires_tap floor, plus the boot-and-test job (`pytest --pyargs tap_plugin.<slug>` in a real compose stack; zero executed tests is red; both SHAs and the count in the job summary) |
| Per-product-line CI lanes (free GitHub runners) | `req-dev-validation-product-line-lanes` | Pre-push (promote-triggered `core_ci`) + CI (every PR; tier-gated — docs-tier diffs skip the lane, specs-tier and up run `core_ci`, req-dev-validation-product-line-lanes-7) + Nightly against `main` (`schedule`, 08:23 UTC; a red `gate` files one owner issue, req-dev-validation-product-line-lanes-12) | Gate-guarded — `core_ci` is the one line and the promote gate; the `test_all` PR line was eliminated 2026-09-10 (tap#369: it walked the core tree and never the plugin suites; the full set runs in the BOM lane), and the `samsite` product line left 2026-09-25 (tap#638: tap-plugin-samsite's own CI owns its record; a core change that rots it is caught nightly by `nightly-plugins.yml`, not on the core PR). Ran on AWS CodeBuild until the measured ~9-min free-runner spike retired it (Terraform/account teardown pending, deliberately last) | `.github/workflows/product-lines.yml` (free `ubuntu-latest` runner: the `core_ci` line); `promote-to-main.sh` Step 2.6 opens the PR and blocks on `gate` (req-dev-multisession-ci-gate) |
...
| Plugin dependency closure scanned for vulnerabilities (per plugin repo) | `req-tap-plugin-extdev-repo-ci-10` | Per-push/PR and nightly in each plugin repo (`plugin-ci.yml` `scan-dependencies`) | CI-guarded, fail-closed on a fixable HIGH/CRITICAL or a scan that did not complete — once a caller's pin reaches it (tap#772). Unfixed advisories are reported, not gated. New advisories are caught at the next run (the nightly: ~24h), not continuously | `tap/dependency_snapshot.py --format cyclonedx` walks the plugin's own closure (stopping at sibling plugins) from the booted venv; pinned Trivy scans it in a job with no write scope; `scripts/release_cve_gate.py --gate plugin-closure` gives the verdict and `--check-waivers` holds the plugin repo's `.trivyignore` to one reason per entry; `scripts/sbom/sarif_locate.py --plugin-subdir` locates each result on the plugin's pyproject.toml; a `security-events: write`-only job uploads it on the plugin repo's default branch. `tap/tests/test_plugin_ci_workflow.py` pins the workflow's shape, including that no job holds `contents: write` (`req-tap-plugin-extdev-repo-ci-11`) |
...
| Release attestation gate (a version tag requires verified attestations) | `req-cicd-sbom-4` | Per-release-tag (`publish-release-tags.yml` `retag`, before the version tag is created) | CI-guarded (fail-closed at release) | `.github/workflows/publish-release-tags.yml` verifies provenance on the index digest and both SBOM predicates on every per-arch child, refusing the promotion on MISSING or NOT OBSERVABLE; `tap/tests/test_release_gates.py` asserts the ordering and executes the gate body against stubs (tag-push workflows cannot be exercised by PR CI — tap#525). |
| Release candidate passes core_ci (a version tag requires the core_ci line + cold-boot) | `req-dev-validation-product-line-lanes-11` | Per-release-tag (`publish-release-tags.yml` `core-ci-line` + `core-ci-cold-boot`, before `candidate` and `retag`) | CI-guarded (fail-closed at release) — the same `core-ci.yml` definition every PR runs, on the tag's commit; a tap release gates on core only, never on a product or the `test_all` union (tap#638) | `.github/workflows/core-ci.yml` (one definition, `lane` input; unknown lane fails in `lane-check`); `tap/tests/test_product_lines_gate.py` asserts the release calls both lanes on `github.ref` and that `candidate`/`retag` need them (tag-push workflows cannot run on PR CI) |
| Release CVE gate (a version tag requires no fixable High/Critical) | `req-cicd-base-image-lifecycle-2` | Per-release-tag (`publish-release-tags.yml` `release-cve-scan`, before the version tag is created) | CI-guarded (fail-closed at release) | Pinned Trivy (`--severity HIGH,CRITICAL --ignore-unfixed`, `.trivyignore` from the tagged commit) in a job with no write scope; `limit-severities-for-sarif` is required for the severity scope to reach SARIF at all (tap#824), held for every severity-scoped SARIF scan by `tap/tests/test_trivy_sarif_severity.py`; `scripts/release_cve_gate.py` splits findings from a scan that never ran, both blocking; `tap/tests/test_release_cve_gate.py`. The tip scan (`publish-images` `scan`, `trivy-nightly`) stays report-only by ruling. |
...
| SBOM canary guard (TAP-specific truths) | `req-cicd-sbom-7` | Per-publish (publish-images `sbom` job, before the first-party `attest-sbom` job) | CI-guarded | `scripts/sbom/generate.py` `check_canaries` before attestation; `tap/tests/test_sbom_generate.py` |
| SBOM conformance (schema + minimum elements) | `req-cicd-sbom-11` | Per-publish (publish-images `sbom` job, before the first-party `attest-sbom` job) | CI-guarded | `scripts/sbom/generate.py` fail-closed gates before attestation; `tap/tests/test_sbom_generate.py` |
...
**As built (Phase 1).** The gate is `manage.py cold_boot_gate` (in `tap_boot`), the single artifact local dev, `scripts/gate`, and `scripts/promote-to-main.sh` all invoke identically. `scripts/gate` provisions a **fresh scratch database** on the running stack's Postgres (a management command cannot cleanly `migrate`-from-zero its own connection), points the command's `DATABASE_URL` at it, and drops it on exit — so the cycle runs inside the existing compose image (req-...-5) without a second stack or touching the session DB. The ordered steps: `schema:migrate` (createcachetable→migrate from zero) → `schema:makemigrations` (`--check`) → `profiles:resolve` (every *installable* profile resolves; see the per-profile axis below) → `seed:boot-profile` (strict boot of the gate's profile: `--profile`, defaulting to the stack's `TAP_BOOT_PROFILE`; the CI `cold-boot` job boots `core_ci`, whose population seeds the `validation_sample` fixture's GRIFT. Named `seed:boot-base` before the 2026-07-03 baseline flip and `seed:boot-test_all` until tap#638 made the profile a parameter) → `collector:cycle` (real backend + in-process drain + PRODUCED_BATCH assertion, scheduler evaluated in the same drain) → `health`. **Measured wall-clock: ~66–107s** (green path). Because `seed:boot-profile` boots a whole profile, the gate needs that profile's plugins installed: the promote runs it with `--skip-if-not-installable`, so a stack that has not installed the named profile skips it loudly and the CI `cold-boot` job owns cold-boot truth (`req-dev-validation-smoke-gate-8`). The union still passes through the same gate in the BOM lane (`bom-boot.yml` boots `test_all` and runs `scripts/gate`) until that lane is retired (tap#638). `profiles:resolve` is install-aware through the same shared filter (`tap_boot.profile.installable_profile_ids`) that backs both the pytest profile-resolution guard and this skip predicate. The collector fired is the **deterministic offline canary** `grid_fixtures:canary` (`--collector` overrides it to drive a real domain collector): a `CollectorBase` in the neutral `grid_fixtures` fixtures plugin that emits a fixed two-node/one-edge `grid_fixtures__*` GRIFT batch from inline constants — **no network, no credentials, no filesystem read** — so the gate's real-backend cycle positively asserts grid mutation without ever flaking on an upstream being down. It is self-contained (emits its own plugin's vocabulary) and, like all of `grid_fixtures`, installs only into dev/test profiles, never a lean customer profile. Per-commit guard: `plugins/grid_fixtures/tap_plugin/grid_fixtures/tests/test_canary_collector.py`.
...
| req-dev-validation-smoke-gate-2 | Seed is strict | Implemented | Strict seed; any failed bundle fails the gate. | Step `seed:boot-profile` runs the real boot of the gate's profile (`--profile`, default `TAP_BOOT_PROFILE`; CI boots `core_ci`, which seeds the `validation_sample` fixture's bundle — `tap/tests/test_core_ci_profile.py` refuses an empty population or a seeded plugin with no GRIFT, so the step cannot pass by importing nothing). Population seeds through the same `seed_plugin` op `import_plugin_grift` uses; aligns with `req-dev-multisession-spawn-import-strict-1`. Named `seed:boot-base` before the 2026-07-03 baseline flip and `seed:boot-test_all` until tap#638. |
...
| req-dev-validation-smoke-gate-8 | Narrower-stack skip | Implemented | Step `seed:boot-profile` boots the gate's whole profile (`--profile`, default `TAP_BOOT_PROFILE`), so the gate needs every plugin that profile names installed, plus the plugin that registers the fired collector (the key's scope; `grid_fixtures` for the default canary). On a stack missing either the promote invokes it with `--skip-if-not-installable`; the gate emits a loud SKIP naming the profile and exits 0, delegating cold-boot truth to the CI `cold-boot` job, which boots `core_ci` on its own runner. With neither `--profile` nor `TAP_BOOT_PROFILE`, or with a profile id that names no boot record, the gate refuses to run rather than guess or skip. | The predicate is `<profile> in installable_profile_ids(...)` — the same shared filter behind the install-aware pytest lane (`req-dev-validation-collection-complete-4`) and the profile-resolution guard. A standalone `scripts/gate` (no flag) still runs fully — the skip is opt-in, so the gate stays honest for a full manual check. Until tap#638 the predicate was fixed to the `test_all` union. `tap_boot/tests/test_cold_boot_gate_manifest.py`. |
...
| req-dev-validation-collection-complete-4 | Install-aware for plugins | Implemented | Plugin tests live inside the package (`tap_plugin/<slug>/tests/`). The repo-root walk NEVER collects them (site-packages is a dot-dir; editable checkouts are `--ignore`d): each lane names every installed plugin's suite through the one seam (`tap.plugin_testing` → `tap.lane_run`) and runs it as its own owner; EXPECTED membership derives from the lane's boot record and an undiscovered expected plugin is red (`tap/tests/test_lane_run.py`). Fully strict in the BOM lane (nothing uninstalled); a focused stack owns only what it booted, with the rest owned by the plugin repos' own CI (`req-tap-plugin-extdev-repo-ci`). | Not a silent narrowing: the delegation is to named, gated lanes. Distinct from `_IGNORED_DIRS` (permanent, hand-listed holes) — this hole is dynamic per stack. Before tap#369 the walk was CLAIMED to collect installed plugins' tests and did not. |
...
| req-dev-validation-ratchet-harness-5 | Baseline entries stay in-repo | Implemented | Reading any baseline fails outright on an entry whose path token is absolute, escapes via `..`, or falls under `DEFAULT_EXCLUDE_DIRS` (`tap_secrets` above all) — baselines are committed to a public repo, and the sync workflow is a transport from locally-observed paths into git history; this tripwire fires before the promote gate can push one. Lexical by design (no git/filesystem dependency). | `read_baseline_set` in `tap/ratchet.py`; `tap/jsonfiles.py` `_read_baseline` routes through it. Entries without a `/` path token (RIDs, counts) are not judged. |
...
(the `gryphon_playground` plugin's gridkin scenarios, 7–18 min). Rather than per-test impact analysis
...
- **Footprint** (conservative — errs toward *running*, because the executor compiles
  ...
  false-green, precisely what the corpus exists to catch): `tap_grid/` and
  `tap_api/routers/gryphon.py` (`GRYPHON_FOOTPRINT_RE` in `scripts/test`). The two
  monorepo plugin dirs the footprint named when it was built left this tree with the
  2026-07-21 eviction; the corpus is
  now collected through the installed package (`tap.plugin_testing`, tap#364/tap#370),
  and a plugin's own changes run its corpus in the plugin's own repo.
...
- `--fast` remains the unconditional force-skip; `--gryphon` is the new unconditional
  force-run; `--fast-relevant` (2026-08-31, #254) applies the relevance gate by explicit
  request — fast lane, corpus included exactly when the footprint is touched — and is the
  promote's local lane. An undeterminable merge-base (detached/shallow clone) or a
  non-interactive invocation *with no explicit mode* defaults to **running** the corpus
  (fail toward correctness); `--fast-relevant` asked for the gate by name, so
  non-interactivity does not force-include.
- **Gate safety** (`req-dev-validation-suite-tiers-4`): a relevance-skip can never
  weaken the gate. CORRECTED 2026-08-31 (#254) — this bullet previously claimed the
  promote invokes `--gryphon` (force-full), which was true only in the cloud-inactive
  fallback; the everyday promote invoked `--fast` (unconditional local skip, corpus
  deferred entirely to the cloud gate). A stale claim reading as a guarantee is the
...
| req-dev-validation-suite-tiers-4 | Impact lane is not a gate | Implemented | Any test-impact/affected selection accelerates the inner loop only; the AUTHORITATIVE gate always runs the full lane. | Counters the substitution-backend blind spot. As corrected 2026-08-31 (#254): the authority is the server `gate` (full corpus, unconditional); the promote's LOCAL lane is `--fast-relevant` (corpus when the footprint is touched — earlier feedback, never the gate) in server-gate mode and `--gryphon` (force-full) only in the cloud-inactive fallback, where local IS the authority. The all-plugins CI lane runs `pytest -n 4` directly (never via `scripts/test`), and a non-interactive `scripts/test` with no explicit mode force-runs — so no AUTHORITATIVE path can inherit a relevance-skip. This row previously claimed the promote always calls `--gryphon`; the everyday promote called `--fast` (total local skip, corpus deferred to the cloud) — a stale guarantee, the presence-is-not-correctness shape. |
| req-dev-validation-suite-tiers-5 | Relevance-gated corpus | Implemented | The default local lane runs the Gryphon corpus only when the diff since `origin/main` (merge-base through working tree, incl. untracked) touches the executor footprint; `--fast` force-skips, `--gryphon` force-runs, `--fast-relevant` applies the gate by explicit request (the promote's local lane, #254); an undeterminable base or a non-interactive run with no explicit mode defaults to running it. | Coarse path-based first increment of the affected lane, scoped to the one dominant-cost corpus. Footprint (conservative, errs toward running): `tap_grid/`, `tap_api/routers/gryphon.py` — `GRYPHON_FOOTPRINT_RE` in `scripts/test`; the two monorepo plugin dirs it named at build time left the tree with the 2026-07-21 eviction (tap#364, tap#370), and the corpus is collected through the installed package. Local-interactive accelerator only — never weakens the gate (`suite-tiers-4`). `scripts/test`. |
...
reds the run, which no gate consumes. **Where the red is read:** the double-tap landing
page renders every `github_core__github_actions_run` github_core collects org-wide
(event, conclusion, age), so a red nightly — and a nightly that never ran — is visible
where the operator already looks; nothing files an issue. The triage steps sit in the
workflow file beside the flag (download the artifact, reproduce by seed, fix, and — if
worth locking in — bump the gate's pinned seed). Before 2026-09-11 the lane was
report-only (`fail_on_findings: false`, a `::warning::` + artifact) — a discovery lane
whose discoveries reached nobody who was not already on the Actions tab, which is
presence-not-correctness in lane form. On 2026-09-11 an issue-filing reporter job was
built for this lane and, on 2026-09-14, withdrawn by operator ruling (tap#439): the
landing page is the nerve centre, not a new inbox. This is still where NEW bugs get
...
must read~~ — closed 2026-09-11: a finding reds the run; surfacing that red (and a
schedule that never fires, tap#440) on the landing page is the open half; (2) a
...
| req-dev-validation-api-fuzz-4 | Random-seed exploration lane | Implemented | `api-fuzz-nightly.yml` runs the reusable `api-fuzz.yml` nightly with an empty (random) seed, `max_examples: 200`, `fail_on_findings: true` — discovers NEW bugs off the promote path; a finding reds the run (which no gate consumes), and the red is read on the double-tap landing page from the collected `github_actions_run`s. | One reusable workflow; the gate and this lane are two callers, cannot drift. `fail_on_findings: false` and the `::warning::` posture were the state from 2026-08-11 to 2026-09-11; an issue-filing reporter was built that day and withdrawn 2026-09-14 by operator ruling (tap#439) in favour of the landing page. |
...
This requirement exists because of a concrete miss. `boot/core.boot.json` is `"plugins": []` by design (the product baseline installs nothing), while ~30 core test files import `tap_plugin.grid_fixtures` with no guard — because the invariant lived only as prose, in `tap.plugin_testing`'s docstring (*"the always-installed `core_dev` baseline … never needs guarding"*) and in `core.boot.json`'s own description. Nothing derived it. On any profile without the vocabulary the suite produced ~30 unexplained `ImportError`s, none naming the cause or the fix. Focused product-line sessions hit this on every promote.

#### Implementation

- **One declaration.** `tap.plugin_testing.BASELINE_PLUGIN_SLUGS` is the executable statement of which vocabularies the core suite is written against.
- **Derived, not proxied.** `missing_baseline_plugins()` reads `installed_plugin_slugs()` — the same `TAP_PLUGINS` resolution `tap.settings` uses for `INSTALLED_APPS`, so the harness and settings cannot disagree. Neither `profile_kind` nor `TAP_TEST_MODE` is consulted: both can be true while the vocabulary is absent (`git_serious` is `dev_local` and carries none), and `TAP_TEST_MODE` is true in *every* pytest run by construction, including the failing ones. A classification is not the fact.
- **Fails, does not skip.** The gate is `pytest_collection_modifyitems` in `tap.pytest_harness` (the harness rides `addopts`, so it arrives in every invocation mode — a conftest does not, `req-tap-test-fixtures-4`). It raises `pytest.UsageError` once, naming the missing slugs, what is installed, and the profile to boot. `requires_plugins`' skip disposition is correct for an *optional* plugin and wrong here: skipping would report green having exercised none of the spine, and that green feeds the promote gate.
- **Scoped to core runs.** Only fires when core-located items are collected, so `pytest --pyargs tap_plugin.<slug>` (plugin-CI lane 2) is unaffected — a plugin's own profile has no reason to carry core's fixture vocabulary.

#### Development

The profile convention this enforces is the `core` / `core_dev` split: the product baseline installs nothing, and its `_dev` counterpart adds the fixture vocabulary so the suite can run. Every product line inherits that split — a line whose `_dev` profile omits the baseline cannot run the core suite, and this gate is what says so out loud instead of leaving it to be rediscovered.

#### Acceptance Criteria

| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-baseline-vocabulary-1 | Declared once | Implemented | The baseline set is a single value (`tap.plugin_testing.BASELINE_PLUGIN_SLUGS`), not prose repeated across modules and boot profiles. | Replaced two prose assertions that no code read. |
| req-dev-validation-baseline-vocabulary-2 | Derived from the installed set | Implemented | Missingness is computed as baseline-minus-`installed_plugin_slugs()`, the same resolution `INSTALLED_APPS` uses. | Presence-is-not-correctness: a profile classification that *exists* is not the fact being asserted. |
| req-dev-validation-baseline-vocabulary-3 | Fails loudly, once | Implemented | A core run without the vocabulary raises a single `pytest.UsageError` naming the missing slugs, the installed set, and the profile to boot. | Replaces ~30 unexplained collection errors. Deliberately not a skip. |
| req-dev-validation-baseline-vocabulary-4 | Scoped to core runs | Implemented | The gate fires only when core-located items are collected; plugin-only invocations (`--pyargs`) are unaffected. | Path-based scoping; in-worktree plugin checkouts (`plugins/`, `_dev-plugins/`) are other repositories, not core. |
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-promote-hook-1 | Gate before push | Implemented | The promote path runs the gate after the pre-push merge and before the atomic push. | `scripts/promote-to-main.sh` Step 2.5 (after merge, before atomic push). Cloud-inactive: full lane (`scripts/test --gryphon`) → cold-boot → lean-boot, local as sole authority. Server-gate mode: local `--fast-relevant` lane (#254) as fast feedback, with the required server `gate` (full corpus + boot gates) the blocking authority. Either way the tree that becomes `origin/main` is validated by a full lane before merge. |
...
sharding flakiness because each lane is a deterministic, smaller profile. Since tap#366
the everyday PR lane is `core_ci` (core + the fixture plugins; the github_core canary left it with tap#638); the `test_all` **PR line was
eliminated 2026-09-10** (tap#369, George's ruling: do what it says on the tin or go — it walked the
core tree and never the installed plugin suites, so its green claimed a full set it did not run). The
full set is proven where the BOM matters ([The BOM Lane](#the-bom-lane)); `boot/test_all.boot.json`
stays as the BOM that lane boots.

#### Suite ownership

Every lane's collection derives from ONE seam — `tap.plugin_testing` consumed by `tap.lane_run` —
never from a repo-root walk that happens to find things: pytest's directory collectors prune a
named path under an ignored or dot directory once the root is also an argument, so the
single-invocation shape `pytest <plugin dirs> /app` collects the core walk alone (measured
2026-09-10: 4066 items with or without six seam dirs; the dirs alone, 597). The runner runs the
core walk (minus every plugin dir under the root) and each installed plugin's shipped suite as
separate invocations, derives EXPECTED membership from the lane's boot record, and reports
collected / passed / failed / skipped / deselected / executed per owner in the job summary.

| Lane | Boots | Owns (suites it must execute) | Red when |
| --- | --- | --- | --- |
| `core_ci` (PR gate via `product-lines.yml`; release gate via `publish-release-tags.yml`; one definition, `core-ci.yml`) | `boot/core_ci.boot.json` | core + the three fixture plugins, each by owner; the Gryphon corpus (`gryphon_playground`) is `--require`d to execute | an expected plugin is not discovered; a suite with test files collects 0; a suite collects but executes 0; the corpus not executing; pytest fails |
| `bom-boot` (boot tier via `gate`, nightly) | `boot/test_all.boot.json` — the BOM | core + EVERY installed plugin's shipped suite; the Gryphon corpus (`gryphon_playground`) is `--require`d to execute | as above, plus the corpus not executing |
| plugin repo CI (`plugin-ci.yml`) | the plugin's in-package `ci` record at its `requires_tap` floor | that plugin's own suite (`pytest --pyargs tap_plugin.<slug>`) | 0 executed tests (tap#365) |
| `scripts/test` (local) | whatever this stack booted | core + the booted profile's installed suites; `--gryphon` requires the corpus, `--fast` excludes it; a focused invocation (a path, a node id, `-k`) goes straight to pytest | the runner's rules |


**The `samsite` line left tap's PR checks on 2026-09-25 (tap#638; George's approval of the
removal that day).** tap-plugin-samsite owns its record (`req-boot-bootstrap-samsite-rehome`), and
its own CI boots its in-package `ci` record and runs its shipped suite — including the record
cold-resolve, `tap_plugin/samsite/tests/test_boot_record_resolves.py` (`-rehome-4`) — green on its
`main` at `56c1e29b`. The earlier precondition (tap#364's review amendment: delete the line only after
the product-journey successor, tap-plugin-samsite#4, is observed green) was **not** met: that
journey workflow does not exist yet, and #4 stays open as its own work. What tap gives up is named,
not hidden: a core change that rots the samsite record is no longer caught on the core PR that
causes it — it is caught by `nightly-plugins.yml`, which runs samsite's plugin CI against core `main`
every night (samsite legs green on run 36148706436), and by samsite's own CI on its next change.
...
| Sub-RID | Name | Status | Acceptance | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-product-line-lanes-1 | One lane per product line | Implemented | Each product line (profile) validated in tap's checks has a lane that boots *that* profile and runs *that* line's tests; lines run in parallel. Since tap#638 the only line here is `core_ci`, called from `product-lines.yml` as the reusable `core-ci.yml` (a new line would be a new caller job); product lines (`samsite`) are proven in their own repositories' CI. | Meaningful (product) axis. `test_all` is the union superset lane. Both lanes proven green on CodeBuild (2026-07-08) and on free runners (2026-08-10 spike + first flipped gate run). |
...
| req-dev-validation-product-line-lanes-6 | Promote-gate + Map row when live | Implemented | The `core_ci` lane is the promote gate (`promote-to-main.sh` Step 2.6 opens the PR and blocks on `gate`, option B, reciprocal of `req-dev-multisession-ci-gate`). History: the `test_all` union lane held this role from 2026-08-10 (superseding the free-runner `all-plugins.yml`, retired tap#364) until 2026-09-10, when tap#369 eliminated it. Map row via `DECLARED_SURFACES` ("Per-product-line CI lanes (free GitHub runners)"). | The gate bootstrap-skips the one promote that first lands `product-lines.yml` on `origin/main`, by construction (same detection as the original all-plugins gate). |
| req-dev-validation-product-line-lanes-7 | Change-tier gating (docs/specs shortcut) | Implemented | The `setup` job classifies the diff vs `origin/main` with `scripts/change-tier` (fail-closed: empty/missing-base/unknown paths → `full`): **docs** (inert documentation only — `docs/`, `plan/`, root `*.md`, LICENSE/NOTICE, and skill PROSE `*/skills/*.md`) skips the product lanes, boot gates, and api-fuzz; skill markdown joined the docs tier 2026-09-10 (tap#410) because no lane opens a `SKILL.md` — the full battery cost ten minutes and asserted nothing about it. The `.md` is load-bearing: a skill directory is not prose-only (`tap_web/skills/drive-browser/` ships `drive.py` and `mint_session.py`), and a blanket `*/skills/*` would give executable code a prose-speed gate. What the docs lane SHOULD assert about a skill — that the paths, scripts and RIDs it cites resolve — is proposed in tap#409, and matters more now that this is the only gate a skill gets; **specs** (any `specs/` markdown — parsed by tests: the Map-sync meta-test, RID resolution) runs the `core_ci` lane (every spec-consuming test is a core test, which that lane's core walk contains by construction — no curated subset to maintain) and skips the boot gates + non-core lanes; **full** runs the whole battery. The `gate` aggregator accepts a skip ONLY where the tier justifies it; any other skip, a cancelled job, or a failed `setup` remains a failure. `secret-scan` and `dco` are always-on: docs leak credentials too, and docs commits still need sign-offs. The promote's local fast lane applies the same tier (docs → skip local pytest/boot; never in sole-authority mode). | Same PR flow, same single required check — retires the loud admin direct-push as the everyday docs shortcut. Trust note: the classifier runs from the PR's tree, the same trust model as every check on `pull_request`; the merge-queue endgame (`merge_group` runs the base branch's workflow) is the enforcement backstop for untrusted PRs. |
| req-dev-validation-product-line-lanes-8 | The `core_ci` line IS the PR gate | Implemented | The PR gate runs the `core_ci` line — `boot/core_ci.boot.json`: core + the three fixture plugins the core suite is built on (grid_fixtures, gryphon_playground, validation_sample), and no product component — through `tap.lane_run` (core walk + each plugin suite by owner, membership from the record, execution in the summary), with the Gryphon corpus `--require`d to execute on every PR. The `cold-boot` job boots the same `core_ci` profile on the `full` and `boot` tiers and runs `scripts/gate` over it (`seed:boot-profile`); `gate` accepts its skip only on the docs/specs tiers. The `test_all` PR line was eliminated 2026-09-10 (tap#369/#374) once every displaced plugin suite had been observed green in its own repo (tap#365). github_core and its dependency closure left `core_ci` with tap#638: product components are proven in their products' lanes. | tap#366, tap#369, tap#638. Bounded by `tap/tests/test_core_ci_profile.py` (exactly the fixture plugins; the fixtures' closures stay inside the set; population seeds a fixture that declares GRIFT). The profile owns its pins. Workflow shape: `tap/tests/test_product_lines_gate.py`. |
| req-dev-validation-product-line-lanes-10 | The battery is base-independent | Implemented | `product-lines.yml`'s `pull_request` trigger carries **no `branches:` filter**, so every PR runs the battery whatever its base. A `branches: [main]` filter matches the PR's BASE, and with it a stacked PR — which the multi-session workflow recommends for dependent work — fired no workflow at all: no `setup`, no lanes, no `gate`. Observed 2026-09-16 on two PRs with **zero** product-lines runs each, one sitting `BLOCKED` on a `gate` that could never arrive and the other reading **`CLEAN`** while never having been tested. Retargeting to `main` does not recover it: a base change emits `pull_request` `edited`, which is not in the default type set. Safe because tiering does not key off the PR base — `setup` runs `scripts/change-tier origin/main`, always against main, so a stacked PR is measured against main and gets a tier at least as high as it would have, never lower. | tap#484. **Do not add `edited` to recover a retarget**: `on:` cannot narrow it to base-changes-only, and `gate` asserts `setup` succeeded, so a title-only edit would skip every job and turn a healthy PR permanently red. Nor make `gate` tolerate absent lane results — a workflow that never ran has no `gate` job to be tolerant with. The cost is CI on non-main-base PRs, bounded by `concurrency: cancel-in-progress` and the change tiers. |
| req-dev-validation-product-line-lanes-9 | The `boot` tier | Implemented | `scripts/change-tier` prints `boot` when any input of the bill of materials is in the diff — derived from the ONE declaration `tap/bom_inputs.py` (`BOM_INPUTS`: the boot records, `uv.lock`, `pyproject.toml`, `docker/Dockerfile*`, the entrypoint, the OpenSSL FIPS provider pin + key file, `docker-compose*.yml`, `.env`, `tap/preboot.py`, `tap_boot/`, and anything under a record's editable/path source), never from globs restated in the script; every consumer treats `boot` as `full` and more — the lanes and boot gates run, and the BOM lane (`req-dev-validation-bom-lane-2`) is additionally required through `gate`; `promote-to-main.sh` reads the same tier and runs the local boot gates on it. Fail-closed like the other tiers. | tap#366, tap#379 (a lockfile-only PR, PR# 373, was `full` under the two-glob proxy). `tap/tests/test_change_tier.py` (every declared input + an editable path), `tap/tests/test_bom_inputs.py` (drop-one, the boot's read set is declared, the workflows' uv-cache `hashFiles` equal the declared `RESOLUTION_INPUTS`). |
| req-dev-validation-product-line-lanes-11 | A release gates on core_ci, defined once | Implemented | `publish-release-tags.yml` runs the `core_ci` line and the cold-boot gate on the exact tag commit (`ref: github.ref`) before `candidate` and `retag`; a red leaves the version tag unpublished. Both run from ONE definition, `.github/workflows/core-ci.yml` (`workflow_call`, inputs `ref` and `lane`), which `product-lines.yml` also calls for its `line` and `cold-boot` jobs — so a release is gated on exactly what every PR is gated on, and no copy of the job bodies exists to drift. One lane per call keeps a separate result per lane for `gate`; an unknown `lane` fails in `lane-check` rather than skipping every job into a green call. A tap release gates on core only, never on a product or the `test_all` union. | Ruling George 2026-09-19, restated 2026-09-25; tap#638. Supersedes `req-dev-validation-bom-lane-3`. `tap/tests/test_product_lines_gate.py` (`test_a_release_gates_on_core_ci_not_on_the_union`, `test_core_ci_is_defined_once_and_fails_closed_on_an_unknown_lane`). Release-only workflows cannot run on PR CI; the first real exercise is the next `v*` tag. |
| req-dev-validation-product-line-lanes-12 | Main is proven nightly | Implemented | `product-lines.yml` carries a `schedule` trigger (cron `23 8 * * *`, clear of every other cron in the repo), so the whole battery runs against `main`'s own content once a day. On `schedule` the ref is `main`, so `setup`'s `scripts/change-tier origin/main` sees an empty diff and fails closed to `full` (every lane, boot gate and api-fuzz runs), and `gate`'s independent BOM step classifies the empty list `no-boot` (bom-boot is not required; it has its own nightly). A job AFTER `gate`, `owner-issue`, runs only on `schedule`: a `failure` files ONE open issue ("Nightly red: tap main") or comments on the existing one, a `success` closes it, and a cancelled or skipped `gate` changes neither. The existing issue is matched by author (the Actions bot) AND a hidden body marker, never by title; the listing passes `--limit` and fails if it reaches it; a `concurrency` group serialises the job. It holds `issues: write` only and checks out no repository code. There is deliberately no `push` trigger. | tap#791 (maintainer approval 2026-09-25: schedule yes, push no, the issue-filing job yes). Every PR runs against its merge ref, so each change was tested against main-plus-itself — what had no coverage was the interaction of merges and baselines drifting on main (a mypy baseline 80 vs 84 on `origin/main`). `owner-issue` is the one job outside `gate.needs`, held to exactly that shape by `tap/tests/test_product_lines_gate.py`. Its behaviour is unobserved until the first scheduled run: GitHub does not fire `schedule` on a PR, and a `workflow_dispatch` skips the job by design. |
...
**The boot record is the known-good-set (BOM) the lane verifies** — the one sentence carried over
from the superseded `req-dev-validation-all-plugins-lane`. `boot/test_all.boot.json` pins every
plugin's source + rev with an integrity digest; whether that SET boots together and passes its
tests is a property of the record, not of every core PR. Jenkins runs its Plugin Compatibility
Tester from the `bom` repository against core releases, not on core PRs; pytest's `plugincompat`
is a scheduled matrix beside the project. Same shape here (tap#363, stage 3; tap#366).

#### Implementation

- `.github/workflows/bom-boot.yml` — `workflow_call` (input `ref`), `schedule` (nightly), `workflow_dispatch`.
  Boots `test_all` in the real image (the `product-lines.yml` boot idiom), runs `scripts/gate`, builds the
  per-profile template DB, then runs the WHOLE suite: core plus every installed plugin's shipped tests, each
  named through `tap.plugin_testing` because the repo-root walk never enters `site-packages` (tap#369). A
  plugin that contributes zero tests fails the lane — a silent skip is the false green this lane removes.
- **Boot-record change**: `product-lines.yml` `bom-boot` job calls the workflow when `scripts/change-tier`
  says `boot`; the `gate` aggregator requires it for that tier and accepts its skip on every other tier.
- **Release candidate — no longer this lane.** Until 2026-09-25 `publish-release-tags.yml` called it on
  the tag's ref before `retag`. A tap release now gates on core only (`req-dev-validation-product-line-lanes-11`,
  tap#638): the same `core-ci.yml` line + cold-boot gate every PR runs, on the tag's commit.
- **Everyday PRs** run `core_ci` (`req-dev-validation-product-line-lanes-8`); this lane
  is not on their path unless the BOM moved.

#### Acceptance Criteria

| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-bom-lane-1 | Full Set, Whole Suite, By Owner | Implemented | The run boots `test_all` (the BOM) in the real image, passes `scripts/gate`, then `tap.lane_run` runs the core walk and every installed plugin's shipped suite as separate invocations and writes collected / passed / failed / skipped / deselected / executed per owner to the job summary; the Gryphon corpus (`gryphon_playground`) is `--require`d to execute. Red on an undiscovered expected plugin, a suite that collects 0 with test files present, a suite that executes 0, or the corpus not executing — a collected count alone is not evidence the suite ran. | `bom-boot.yml`; `tap/tests/test_lane_run.py`. The earlier single-invocation shape (`pytest <dirs> /app`) collected the core walk alone; the per-dir counts it printed were `--collect-only` numbers, not executions (tap#369). |
| req-dev-validation-bom-lane-2 | Required When The BOM Moves | Implemented | A PR whose diff touches any declared BOM input (`tap/bom_inputs.py`: boot records, `uv.lock`, `pyproject.toml`, the image, compose, `.env`, the install code, a record's editable path) is classified `boot` and `gate` requires `bom-boot`; on other tiers its skip is accepted. The lane proves ASSEMBLY (the set boots and its suites execute), not behaviour beyond those suites. | `scripts/change-tier` → `tap.bom_inputs --classify`, `product-lines.yml` `gate`; `tap/tests/test_change_tier.py`, `tap/tests/test_bom_inputs.py`. |
| req-dev-validation-bom-lane-3 | Release Candidate Boots Before It Ships | Retired | `publish-release-tags.yml` runs the lane on the exact tag commit and `retag` needs it; a red leaves the version tag unpublished. | **Retired 2026-09-25 (tap#638):** a tap release gates on core only, never on a product or the `test_all` union (ruling George 2026-09-19, restated 2026-09-25). Superseded by `req-dev-validation-product-line-lanes-11` — the release candidate runs `core-ci.yml`'s line + cold-boot; the gate-the-promotion-not-the-sha-manifest property carries over unchanged. The lane itself (`-1`, `-2`, `-4`) is untouched here; its retirement is separate. |
| req-dev-validation-bom-lane-4 | The Core Contract Is Explicit | Implemented | `tap_plugins/tests/test_plugin_contract.py` runs for every installed plugin: manifest loads; declared models and edge types are registered; required `depends_on` are installed; migrations clean (`makemigrations --check`) and applied; GRIFT bundles import through `seed_plugin` and a plugin-typed node reads back through `services.get_node`; API router mounted under `/api/v1/plugins/<label>/`; panel templates load; collector keys resolve. A non-applicable check skips with the reason. | Runs in `core_ci` (the fixture plugins; validation_sample carries the GRIFT round-trip) and in the BOM lane (full set). |
