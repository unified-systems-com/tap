...
| BOM lane: the full pinned set boots and passes its whole suite | `req-dev-validation-bom-lane` | Nightly (`bom-boot.yml` schedule) + CI when a boot record changes (`boot` tier, REQUIRED via `gate`) + release candidate (`publish-release-tags.yml` before `retag`) + dispatch | Gate-guarded on the boot tier and on release tags; nightly is signal. Boots `test_all` in the real image, runs `scripts/gate`, then core + every installed plugin's shipped tests named through `tap.plugin_testing` (a plugin contributing 0 tests is a red) — the first lane that runs the plugin suites at all since the eviction (tap#369) | `.github/workflows/bom-boot.yml`; `scripts/change-tier` (`boot`); `product-lines.yml` `gate` |
...
| Core PR lane: core + fixtures + one canary (`core_ci`) | `req-dev-validation-product-line-lanes-8` | CI (every PR, `product-lines.yml` `line` matrix entry `core_ci`, beside `test_all`; REQUIRED via `gate`) | Gate-guarded — boots `boot/core_ci.boot.json` (grid_fixtures, gryphon_playground, validation_sample, identity_core, github_core) and runs the core suite + the plugin contract suite; bounded by `tap/tests/test_core_ci_profile.py` (fixtures + the canary closure only; pins equal `test_all`'s). The flip that retires `test_all` from the PR gate waits on tap#365's precondition (tap#366) | `.github/workflows/product-lines.yml` `line` (`core_ci`); `tap/tests/test_core_ci_profile.py` |
...
remains one lane (the union superset); since tap#366 a `core_ci` line (core + fixtures + one canary)
runs beside it as the everyday PR lane, and the full set is proven where the BOM matters
([The BOM Lane](#the-bom-lane)).
...
| req-dev-validation-product-line-lanes-8 | The `core_ci` line beside `test_all` | Implemented | The PR gate runs a `core_ci` line — `boot/core_ci.boot.json`: core + the fixture plugins the core suite needs + one flagship canary (github_core) with its declared dependency closure — beside the `test_all` union; both are required through `gate`. `test_all` keeps its name and stays the full set. The flip (`test_all` leaves the PR gate) is its own issue whose precondition is every displaced plugin suite observed green in its own repo (tap#365). | tap#366. Bounded by `tap/tests/test_core_ci_profile.py`: only fixtures + the canary closure, and every pin equals `test_all`'s. |
...
| ACID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-bom-lane-1 | Full Set, Whole Suite, Nightly | Implemented | The scheduled run boots `test_all` in the real image, passes `scripts/gate`, and runs core + every installed plugin's tests with a per-plugin collected count > 0 printed in the log. | `bom-boot.yml`; observation of the first green nightly is the flip to Observed (its own event). |
