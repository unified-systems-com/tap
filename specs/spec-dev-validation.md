...
| req-dev-validation-product-line-lanes-8 | The `core_ci` line IS the PR gate | Implemented | The PR gate runs the `core_ci` line — `boot/core_ci.boot.json`: core + the fixture plugins the core suite needs + one flagship canary (github_core) with its declared dependency closure — through `tap.lane_run` (core walk + each plugin suite by owner, membership from the record, execution in the summary); `cold-boot` boots the same profile. The `test_all` PR line was eliminated 2026-09-10 (tap#369/#374) once every displaced plugin suite had been observed green in its own repo (tap#365, all 15 `ci` records merged with boot-and-test green at their floors). | tap#366, tap#369. Bounded by `tap/tests/test_core_ci_profile.py`: only fixtures + the canary closure, and every pin equals `test_all`'s. |
