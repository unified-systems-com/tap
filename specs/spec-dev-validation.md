...
| req-dev-validation-bom-lane | [The BOM Lane](#the-bom-lane) | Implemented | The boot record IS the bill of materials: the full pinned set boots in the real image and passes core + every installed plugin's shipped tests — nightly, when a boot record changes (required through `gate`), and on the release candidate before the version tag is promoted (`bom-boot.yml`, tap#366) |
...
| req-dev-validation-product-line-lanes-9 | The `boot` tier | Implemented | `scripts/change-tier` prints `boot` when any `boot/*.boot.json` (core or in-package) is in the diff; every consumer treats `boot` as `full` and more — the lanes and boot gates run, and the BOM lane (`req-dev-validation-bom-lane-2`) is additionally required through `gate`. Fail-closed like the other tiers. | `tap/tests/test_change_tier.py`. |
...
| req-dev-validation-bom-lane-2 | Required When The BOM Moves | Implemented | A PR whose diff touches any `boot/*.boot.json` is classified `boot` and `gate` requires `bom-boot`; on other tiers its skip is accepted. | `scripts/change-tier`, `product-lines.yml` `gate`; `tap/tests/test_change_tier.py`. |
