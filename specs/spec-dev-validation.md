...
| Issue-link trailers (both roads to main) | `req-cicd-issue-link` | Pre-push (`scripts/check-issue-link`, wired into `promote-to-main.sh`) + CI (`product-lines.yml` `dco` job) | Gate-guarded (enforcing from 2026-09-03; tap#327) | `scripts/check-issue-link` (the range over origin/main carries a qualified `Closes:` / `Part-of:` / `No-issue:` trailer); `scripts/promote-pr-body` derives the PR body's `## Issues` lines from the same parser, so GitHub auto-links and auto-closes; the CI bot exemption is by verified identity from `tap/tap.pr-bots.json` (`req-cicd-issue-link-6`) |
