...
| BOM lane: the full pinned set boots and passes its whole suite | `req-dev-validation-bom-lane` | Nightly (`bom-boot.yml` schedule) + CI when a boot record changes (`boot` tier, REQUIRED via `gate`) + release candidate (`publish-release-tags.yml` before `retag`) + dispatch | Gate-guarded on the boot tier and on release tags; nightly is signal. Boots `test_all` (the BOM) in the real image, runs `scripts/gate`, then `tap.lane_run`: the core walk and every installed plugin's shipped suite as separate invocations (pytest prunes a named plugin dir once the root is also an argument — the single-invocation shape ran core alone), membership derived from the record, per-owner collected/executed in the summary, the Gryphon corpus required to execute (tap#369) | `.github/workflows/bom-boot.yml`; `scripts/change-tier` (`boot`); `product-lines.yml` `gate` |
...
| --- | --- | --- | --- |
| `core_ci` (PR gate, `product-lines.yml`) | `boot/core_ci.boot.json` | core + the three fixture plugins, each by owner; the Gryphon corpus (`gryphon_playground`) is `--require`d to execute | an expected plugin is not discovered; a suite with test files collects 0; a suite collects but executes 0; the corpus not executing; pytest fails |
| `bom-boot` (boot tier via `gate`, nightly, release candidate) | `boot/test_all.boot.json` — the BOM | core + EVERY installed plugin's shipped suite; the Gryphon corpus (`gryphon_playground`) is `--require`d to execute | as above, plus the corpus not executing |
...
| Sub-RID | Name | Status | Acceptance | Notes |
| --- | --- | :---: | --- | --- |
| req-dev-validation-product-line-lanes-1 | One lane per product line | Implemented | Each product line (profile) validated in tap's checks has a lane that boots *that* profile and runs *that* line's tests; lines run in parallel. Adding a line = one workflow matrix entry. Since tap#638 the only line here is `core_ci`; product lines (`samsite`) are proven in their own repositories' CI. | Meaningful (product) axis. `test_all` is the union superset lane. Both lanes proven green on CodeBuild (2026-07-08) and on free runners (2026-08-10 spike + first flipped gate run). |
...
- **Release candidate**: `publish-release-tags.yml` calls it on the tag's ref before `retag` promotes the
  sha manifest to the version tag; red blocks publication.
...
| req-dev-validation-bom-lane-3 | Release Candidate Boots Before It Ships | Implemented | `publish-release-tags.yml` runs the lane on the exact tag commit and `retag` needs it; a red leaves the version tag unpublished. | The sha manifest still exists (publish-images); only the versioned promotion is gated. |
