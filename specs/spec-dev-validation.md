...
| req-dev-validation-smoke-gate-8 | Narrower-stack skip | Implemented | Step `seed:boot-profile` boots the gate's whole profile (`--profile`, default `TAP_BOOT_PROFILE`), so the gate needs every plugin that profile names installed. On a stack that has not installed them the promote invokes it with `--skip-if-not-installable`; the gate emits a loud SKIP naming the profile and exits 0, delegating cold-boot truth to the CI `cold-boot` job, which boots `core_ci` on its own runner. With neither `--profile` nor `TAP_BOOT_PROFILE` the gate refuses to run rather than guess. | The predicate is `<profile> in installable_profile_ids(...)` — the same shared filter behind the install-aware pytest lane (`req-dev-validation-collection-complete-4`) and the profile-resolution guard. A standalone `scripts/gate` (no flag) still runs fully — the skip is opt-in, so the gate stays honest for a full manual check. Until tap#638 the predicate was fixed to the `test_all` union. `tap_boot/tests/test_cold_boot_gate_manifest.py`. |
