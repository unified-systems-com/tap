...
| Core PR lane: core + fixtures + one canary (`core_ci`) | `req-dev-validation-product-line-lanes-8` | CI (every PR, `product-lines.yml` `line` matrix entry `core_ci` — THE PR gate since the `test_all` line was eliminated, tap#369/#374; REQUIRED via `gate`) | Gate-guarded — boots `boot/core_ci.boot.json` (grid_fixtures, gryphon_playground, validation_sample, identity_core, github_core) and runs the core suite + the plugin contract suite; bounded by `tap/tests/test_core_ci_profile.py` (fixtures + the canary closure only; pins equal `test_all`'s). Runs through `tap.lane_run`: the core walk and each installed plugin's suite as separate owners, membership from the boot record, per-owner execution in the job summary (tap#369) | `.github/workflows/product-lines.yml` `line` (`core_ci`); `tap/tests/test_core_ci_profile.py` |
...
the everyday PR lane is `core_ci` (core + fixtures + one canary); the `test_all` **PR line was
...
| --- | --- | --- | --- |
| `core_ci` (PR gate, `product-lines.yml`) | `boot/core_ci.boot.json` | core + the fixture plugins + the github_core canary closure, each by owner | an expected plugin is not discovered; a suite with test files collects 0; a suite collects but executes 0; pytest fails |
...
| req-dev-validation-product-line-lanes-8 | The `core_ci` line IS the PR gate | Implemented | The PR gate runs the `core_ci` line — `boot/core_ci.boot.json`: core + the fixture plugins the core suite needs + one flagship canary (github_core) with its declared dependency closure — through `tap.lane_run` (core walk + each plugin suite by owner, membership from the record, execution in the summary). `cold-boot` keeps booting `test_all`: it is a BOOT gate, not a test lane — its `scripts/gate` run seeds `boot-test_all` internally, so it proves the union still boots from a fresh scratch DB while the PR's tests run on `core_ci`. The `test_all` PR line was eliminated 2026-09-10 (tap#369/#374) once every displaced plugin suite had been observed green in its own repo (tap#365, all 15 `ci` records merged with boot-and-test green at their floors). | tap#366, tap#369. Bounded by `tap/tests/test_core_ci_profile.py`: only fixtures + the canary closure, and every pin equals `test_all`'s. |
...
- **Everyday PRs** run `core_ci` beside `test_all` (`req-dev-validation-product-line-lanes-8`); this lane
