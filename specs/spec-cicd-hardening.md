...
| req-cicd-base-image-lifecycle-2 | Image CVE gate | Implemented | Two tiers, decided by what a red costs. **Release (`publish-release-tags.yml` `release-cve-scan`, tap#526): BLOCKING** — a `:X.Y.Z` tag is refused on a High/Critical **with a fix available** (`--ignore-unfixed`, `.trivyignore` from the tagged commit), and refused just as firmly when the scan did not complete. **Tip (`publish-images.yml` `scan`, `trivy-nightly.yml`): report-only** — `:latest` must not stall on a CVE disclosed overnight. Optional Copacetic stays deferred. | Realizes `req-cicd-security-scanning-4` (2026-08-09). Ruling George 2026-09-17; unfixable CVEs (zlib CVE-2026-85091, tap#491) would otherwise block every release with no action available. Three verdicts via `scripts/release_cve_gate.py`: clean / FINDINGS (named CVEs) / NOT OBSERVABLE. Grype FIPS-provider findings excluded pending tap#295. |
...
| req-cicd-branch-protection-2 | Bypass for the promote identity | Retired | **Retired 2026-08-21 — the bypass list is EMPTY on every ruleset (verified against the API 2026-08-24: `bypass_actors: []` on `main-required-checks`, `org-require-pr`, `protect-default-branches`).** The interim repository-admin bypass existed so the promote's direct atomic push survived the required-check ruleset. PR #101 then showed that same always-bypass had also been silently swallowing the code-owner rule — making `gate` advisory-in-fact for admins all along. Break-glass is now an org owner editing the ruleset itself: audit-logged and loud. | Superseded by the PR flow (`req-dev-multisession-push-workflow`) and by doctrine 4, *the maintainer is not special* (`spec-cicd-ai-review.md`): a maintainer-only affordance is a defect, not a convenience. The emptying was recorded there at the time; this row is the owning requirement catching up. |
...
tolerated. The verdict is per range: one trailer covers a branch's fix-up commits. Merge commits are
exempt; a pull request is exempt only when its authenticated author is an approved bot IDENTITY
(numeric id + type, never a name — `req-cicd-issue-link-6`). "Closes" means the done-test is met by
...
| RID | Title | Status | Description | Notes |
| --- | --- | :---: | --- | --- |
| req-cicd-dco-signoff-1 | Sign-off applied automatically | Implemented | `.githooks/prepare-commit-msg` appends the committer's `Signed-off-by` trailer to every non-merge commit, from the committer's git identity. | Lives in the versioned `.githooks/` dir, installed per-clone by `scripts/hooks-install` (offered by `spawn-session.sh`, runnable directly by a plain-clone contributor — one path for everyone, no maintainer special case). Since 2026-08-28 installation is a deliberate human act, not a spawn side effect (`req-dev-localexec-consent`): `CONTRIBUTING.md` requires the named human signer to authorize the certification, which a silently-armed auto-stamper contradicted. The trailer is applied only where hooks are installed; `req-cicd-dco-signoff-2` is what makes its absence fail. |
| req-cicd-dco-signoff-2 | Trailer check on both roads to main | Implemented | `scripts/check-dco` verifies every non-merge commit added over `origin/main` carries the trailer; wired into the promote's local gates and the `dco` job in `product-lines.yml` (the PR road + the promote's dispatched cloud gate). The ONE exemption is a pull request whose AUTHENTICATED author is an approved bot IDENTITY — GitHub's `pull_request.user.{id,type}` against `tap/tap.pr-bots.json`, the same decision `req-cicd-issue-link-6` makes, derived once in `scripts/pr_bot_identity.py`. Commit author strings are NEVER consulted for the exemption. Where no authenticated identity exists — a local promote (no PR yet), a `merge_group` event — nothing is exempt. | ONE artifact, MANY invokers (the `scripts/gate` pattern). Until 2026-09-18 the exemption matched `(renovate\|dependabot\|github-actions\|tap-release-please)\[bot\]` against `%an <%ae>`, which the committer sets: `git -c user.name='renovate[bot]' commit` produced a commit the gate waved through, leaving a certification in the record that no person made (tap#335, found by the Codex/Grok seats on PR# 328 for the sibling check). The negative control — a commit whose author string merely claims to be a bot is REFUSED — is `test_a_spoofed_bot_author_string_is_not_an_exemption` in `tap/tests/test_check_dco.py`; without it the suite would pass for the wrong reason. Consequence named rather than discovered: the local lane exempts nothing, so it is strictly stricter than the server gate, and an unsigned bot commit carried into a HUMAN's branch is a red — bot changes land on the bot's own PR. |
| req-cicd-dco-signoff-3 | Enforcement flips with the policy | Implemented | Landed 2026-08-12: `CONTRIBUTING.md` + `DCO` at the repo root, and `scripts/check-dco` now FAILS on a missing trailer — the enforcing default lives in the script itself (escape hatch `TAP_DCO_REPORT_ONLY=1`) rather than an env var per invoker, so an ad-hoc run can never be quieter than the gate. | Approval provenance (stated exactly, because this document is itself a provenance instrument): outside counsel reviewed and approved the **contribution terms** (the v1 draft — licensing, prospective grant, employer authority, AI-assisted contributions); the **v2 additions** (contribution process, tests/code-quality bar, and the two sign-off-mechanics paragraphs) were authored in-repo and approved by the **project steward**, not re-reviewed by counsel. Pre-flight before flipping: no open PR and no live session branch carried an unsigned commit (two 3-week/3-month-stale branches did; both long superseded). The red spells out both remediation forms (`--amend -s`, `rebase --exec`) and the hooksPath caveat for plain clones. Composes with `req-cicd-branch-protection`, by a NAMED mechanism rather than an intention: the ruleset requires exactly one Actions context, the `gate` aggregator, so a job is server-enforcing iff it is in `gate`'s `needs` list in `product-lines.yml` and its result is folded into gate's verdict step. `dco` was not, from 2026-08-12 until 2026-09-18 — it ran, it went red, and it blocked nothing, while this row and CONTRIBUTING said the check was enforcing (tap#353; `req-cicd-issue-link-2` rode in the same job and had the same gap). Now `needs: [setup, secret-scan, dco, …]` and gate dies on `dco != success`, never tier-gated: a trailer is a property of the commits, identical on a docs-tier diff. `tap/tests/test_product_lines_gate.py` asserts every job in the workflow is folded into `gate`, so a lane cannot silently drop out again. |
| req-cicd-dco-signoff-4 | Remediation without rewriting history | Implemented | An unsigned commit may be certified retroactively by a later **individual remediation commit** (`I, <name> <email>, hereby add my Signed-off-by to this commit: <sha>`), itself signed, whose declared identity matches BOTH its own author and the target commit's author. `scripts/check-dco` accepts this form; CONTRIBUTING documents it. | Prior art: the reference DCO app (dcoapp/app) supports exactly this, and its stated advantage is the one our amend/rebase advice lacked — history does not change, so a shared branch is not broken under other people. Format and identity rules follow that implementation verbatim so ecosystem knowledge transfers. **Third-party remediation** (`On behalf of X, I, Y, hereby add…`) is deliberately NOT accepted: certifying another person's right to submit is a materially larger statement than certifying your own, and enabling it is a steward policy call, not a mechanical convenience. Behaviorally guarded by `tap/tests/test_check_dco.py` (signed passes, unsigned fails, an approved bot PR is exempt while a spoofed bot author string is not, remediation certifies, cross-identity remediation rejected). |
