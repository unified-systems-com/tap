...
- `TAP-KNOWN-DUPE(boot-source-input-patterns)` — the accepted shapes of the four
  ...
  no whitespace or control characters), git `commit` (40-hex lowercase) and wheelhouse
  `version` (PEP 440 character set). The three git shapes are OWNED by `tap/git_pin.py`
  (it builds a `git ls-remote` argv from url and rev itself, so it validates them itself)
  and re-exported, not re-spelled, by `tap/preboot.py`; the wheelhouse shape is pre-boot's.
  Importable as `GIT_SOURCE_URL_PATTERN` / `GIT_SOURCE_REV_PATTERN` /
  `GIT_SOURCE_COMMIT_PATTERN` / `WHEELHOUSE_VERSION_PATTERN` from `tap/preboot.py`, and spelled again as the anchored `pattern` on
  ...
  diverges. Owned by `req-boot-install-section` in `specs/spec-tap-boot-v0.md`.
